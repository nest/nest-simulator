...
private:
  ...
  friend int glif_lif_r_cond_dynamics( double, const double*, double*, void* );

  // The next two classes need to be friends to access the State_ class/member
  friend class nest::RecordablesMap< glif_lif_r_cond >;
  friend class nest::UniversalDataLogger< glif_lif_r_cond >;
  ...
  struct Buffers_
  {
    ...
    nest::UniversalDataLogger< glif_lif_r_cond > logger_;
  ...
  static nest::RecordablesMap< glif_lif_r_cond > recordablesMap_;

  /** @} */
};

inline size_t
nest::glif_lif_r_cond::Parameters_::n_receptors_() const
{
  return tau_syn_.size();
}

inline nest::port
nest::glif_lif_r_cond::send_test_event( nest::Node& target,
...
inline nest::port
nest::glif_lif_r_cond::handles_test_event( nest::CurrentEvent&,
  nest::port receptor_type )
{
  // You should usually not change the code in this function.
...
inline nest::port
nest::glif_lif_r_cond::handles_test_event( nest::DataLoggingRequest& dlr,
  nest::port receptor_type )
{
  // You should usually not change the code in this function.
  // It confirms to the connection management system that we are able
...
inline void
glif_lif_r_cond::get_status( DictionaryDatum& d ) const
{
...
inline void
glif_lif_r_cond::set_status( const DictionaryDatum& d )
{
