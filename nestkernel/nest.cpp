...
get_statusdict()
{
  ...
  statusdict[ "version" ] = std::string( NEST_VERSION_STRING );
  statusdict[ "exitcode" ] = EXIT_SUCCESS;
  statusdict[ "built" ] = std::string( String::compose( "%1 %2", __DATE__, __TIME__ ) );
  statusdict[ "prgdatadir" ] = std::string( NEST_INSTALL_PREFIX "/" NEST_INSTALL_DATADIR );
  statusdict[ "prgdocdir" ] = std::string( NEST_INSTALL_PREFIX "/" NEST_INSTALL_DOCDIR );
  statusdict[ "prefix" ] = std::string( NEST_INSTALL_PREFIX );
  statusdict[ "host" ] = std::string( NEST_HOST );
  statusdict[ "hostos" ] = std::string( NEST_HOSTOS );
  statusdict[ "hostvendor" ] = std::string( NEST_HOSTVENDOR );
  statusdict[ "hostcpu" ] = std::string( NEST_HOSTCPU );

#ifdef _OPENMP
  statusdict[ "threading" ] = std::string( "openmp" );
#else
  statusdict[ "threading" ] = std::string( "no" );
#endif

#ifdef HAVE_MPI
  statusdict[ "have_mpi" ] = true;
#else
  statusdict[ "have_mpi" ] = false;
#endif

#ifdef HAVE_GSL
  statusdict[ "have_gsl" ] = true;
#else
  statusdict[ "have_gsl" ] = false;
#endif

#ifdef HAVE_MUSIC
  statusdict[ "have_music" ] = true;
#else
  statusdict[ "have_music" ] = false;
#endif

#ifdef HAVE_LIBNEUROSIM
  statusdict[ "have_libneurosim" ] = true;
#else
  statusdict[ "have_libneurosim" ] = false;
#endif

#ifdef HAVE_SIONLIB
  statusdict[ "have_sionlib" ] = true;
#else
  statusdict[ "have_sionlib" ] = false;
#endif

#ifdef NDEBUG
  statusdict[ "ndebug" ] = true;
#else
  statusdict[ "ndebug" ] = false;
#endif
  ...
  statusdict[ "exitcodes" ] = exitcodes;
