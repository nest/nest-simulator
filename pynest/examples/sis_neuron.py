...
This scripts simulates a number of all-to-all connected ``sis`` neurons. One neuron
...
num_neurons = 100
neuron_type = "sis_neuron"
sis_neurons = nest.Create(neuron_type, num_neurons)
sis_neurons.beta_sis = 0.01  # downscale infectivity from default 0.1

# connect sis_neurons all-to-all avoiding self-connections (=autapses)
nest.Connect(sis_neurons, sis_neurons, {"rule": "all_to_all", "allow_autapses": False})

sis_neurons[0].S = 1  # infect zeroth neuron
sis_neurons[1:].h = 1  # set number of infected neighbors of all other neurons to 1

# create recording device
multimeter = nest.Create("multimeter")
multimeter.record_from = ["S", "h"]
nest.Connect(multimeter, sis_neurons)
