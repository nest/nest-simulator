...
the ``Connect`` function using the keyword ``syn_spec`` (synapse
...
``"delay"``, the synaptic model (``"synapse_model"``) and parameters
...
other parameters can be scalars or distributions. In the case of
scalar parameters, all keys take doubles except for
``"receptor_type"`` which has to be initialised with an
integer. Distributed parameters are initialised with a Parameter with
