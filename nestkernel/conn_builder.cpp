...
nest::ConnBuilder::~ConnBuilder()
{
  delete weight_;
  delete delay_;
  for ( std::map<Name, ConnParameter*>::iterator 
	  it = synapse_params_.begin();
	it != synapse_params_.end();
	++it )
    delete it->second;
}
...
void nest::ConnBuilder::check_synapse_params_(std::string syn_name, const DictionaryDatum& syn_spec)
{
  ...
  // throw error if n or a are set in quantal_stp_synapse, Connect cannot handle them since they are integer
  ...
  if ( syn_name == "cont_delay_synapse" )
  {
    if ( syn_spec->known(names::delay) )
      net_.message(SLIInterpreter::M_WARNING, "Connect", 
  ...
  // throw error if no volume transmitter is defined or parameters are specified that need to be introduced via CopyModel or SetDefaults
  if ( syn_name == "stdp_dopamine_synapse" )
  {
    if ( syn_spec->known("vt") )
      throw NotImplemented("Connect doesn't support the direct specification of the "
    ...
    if ( net_.get_num_threads() > 1)
    {
      if ( syn_spec->known(names::c) )
        throw NotImplemented("For multi-threading Connect doesn't support the setting "
    ...
    std::string param_arr[] = { "A_minus", "A_plus", "Wmax", "Wmin", "b", "tau_c", 
                                "tau_n", "tau_plus" };
...
void nest::OneToOneBuilder::connect_()
{
  ...
  {
    net_.message(SLIInterpreter::M_ERROR, "Connect", 
    ...
    N_((*conn_spec)[Name("N")])
{
    ...
    if (not multapses_ )
    {
      if ((N_ > static_cast<long>(sources_.size()*targets_.size())))
        throw BadProperty("Total number of connections cannot exceed product "
    ...
    if (not multapses_)
      throw NotImplemented("Connect doesn't support the suppression of multapses in the "
...
void nest::FixedTotalNumberBuilder::connect_()
{
  ...
  // HEP: instead of counting upwards, we might count remaining_targets and remaining_partitions down. why?  
