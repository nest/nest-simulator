...
#                                            'precise_times': True})
        ...
        for step in range(n_steps):
            ...
            if show_all_nest_trace_samples or np.any(np.abs(t - np.array(pre_spike_times) - delay) < resolution/2.):
                ...
                post_trace_value = nest.GetStatus(post_parrot)[0]['post_trace']
                trace_nest.append(post_trace_value)
                print("For NEST: trace = " + str(post_trace_value) + " at time t = " + str(t))
    ...
    def plot_run(self, trace_nest_t, trace_nest, trace_python_ref, pre_spike_times, post_spike_times, resolution, delay, dendritic_delay, sim_time, fname):
        ...
        ax1.set_ylim([0., 1.])
        ax3.set_ylim([0., 1.])
        ax2.set_ylim([0., np.amax(trace_python_ref)])
        ...
        n_spikes = len(post_spike_times)
        for i in range(n_spikes):
            ax3.plot(2 * [post_spike_times[i] + delay + dendritic_delay], [0, 1], linewidth=2, color="red", alpha=.4)

        ax2.plot(np.linspace(0., sim_time, len(trace_python_ref)), trace_python_ref, label="Expected", color="cyan", alpha=.6)
            ...
            for i_search, t_search in enumerate(reversed(np.array(pre_spike_times) + delay)):
                ...
                if t_search <= t:
                    ...
                    if not traces_match:
                        ...
                        """_pre_spike_times = np.array(pre_spike_times)
    ...
    def test_post_trace(self):
        ...
        for spike_times_idx in range(len(pre_spike_times)):
            fname = "/tmp/traces_" + str(spike_times_idx) + ".png"
