...
nest::step_rate_generator::Parameters_& nest::step_rate_generator::Parameters_::
...
nest::step_rate_generator::Buffers_::Buffers_( const Buffers_&,
  step_rate_generator& n )
  : idx_( 0 )
  , amp_( 0 )
  , logger_( n )
{
...
nest::Time
nest::step_rate_generator::Parameters_::validate_time_( double t,
  const Time& t_previous )
{
  ...
  if ( not t_amp.is_grid_time() )
  {
    ...
    else
    {
      ...
      msg << "step_rate_generator: Time point " << t
...
void
nest::step_rate_generator::update( Time const& origin,
  const long from,
  const long to )
{
  assert(
