...
"""
Test that weights and delays can be set to arbitrary values within the limits set by resolution.

The test creates a grid with a single row and uses linear functions for weights and delays.
Expected delays with resolution 0.04 are 1.0 0.96 0.96 0.92 0.92 0.88 0.84 and expected weights
are 1.0, 0.98, 0.96, 0.94, 0.92, 0.90, 0.88, 0.86, 0.84, 0.82.
"""
...
def network():
    ...
    grid = nest.spatial.grid(shape=[10, 1], extent=[10.0, 1.0], center=[0.0, 0.0], edge_wrap=True)
    src_layer = nest.Create("iaf_psc_alpha", positions=grid)
    tgt_layer = nest.Create("iaf_psc_alpha", positions=grid)
...
def test_source_layer_nodes_dump(tmp_path, network, expected_source_nodes_dump):
    """Test that source layer nodes dump mathces expectation."""

    src_layer, _ = network

    fname = tmp_path / "source_nodes.txt"
...
def test_target_layer_nodes_dump(tmp_path, network, expected_target_nodes_dump):
    """Test that target layer nodes dump mathces expectation."""

    _, tgt_layer = network

    fname = tmp_path / "target_nodes.txt"
...
def test_layer_connections_dump(tmp_path, network, expected_conn_dump):
    """Test that layer connections dump mathces expectation."""

    src_layer, tgt_layer = network
