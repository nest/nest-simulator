...
nest::eprop_iaf_psc_delta_adapt::pre_run_hook()
{
  ...
  if ( P_.surrogate_gradient_ == "pseudo_derivative" )
    compute_surrogate_gradient = &eprop_iaf_psc_delta_adapt::compute_pseudo_derivative;
}
...
double
nest::eprop_iaf_psc_delta_adapt::compute_pseudo_derivative()
{
  double v_m = S_.r_ > 0 ? 0.0 : S_.y3_;
  double v_th = S_.r_ > 0 ? P_.V_th_ : S_.adapting_threshold_;
