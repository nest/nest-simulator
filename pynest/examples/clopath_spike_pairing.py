...
------------------
This script simulates one aeif_cbvg_2010 neuron that is connected to a
Clopath synapse. The synapse receives pairs of a pre- and a postsynaptic
spike that are separated by either 10ms (pre before post) or -10ms (post
before pre). The change of the synaptic weight is measured after five such
...
The setup consists of two spike generators one of which is connected to a
parrot neuron which in turn is connected to the aeif_cbvg_2010 neuron via a
Clopath synapse. The other spike generator is connected directly to the
neuron.
...
# Neuron parameters. To enable voltage dependent prefactor A_LTD(u_bar_bar) 
# add 'A_LTD_const':False to the dictionary.
nrn_params = {  'V_m':-70.6,
                'E_L':-70.6,
                'C_m':281.0,
                'theta_minus':-70.6,
...
# Spike times of presynaptic spike generator
spike_times_pre = [
...
# Spike times of postsynaptic spike generator
spike_times_post = [
        [  10.,  110.,  210.,  310.,  410.],
        [  10.,   60.,  110.,  160.,  210.],
...
for (s_t_pre, s_t_post) in zip(spike_times_pre, spike_times_post):
    ...
    # We need a parrot neuron since spike generators can be connected
    # only with static connections
...
axA.plot([10., 20., 30., 40., 50.], syn_weights[5:], color = 'b', lw = lw, ls = '-',
        label = "pre-post pairing")
axA.plot([10., 20., 30., 40., 50.], syn_weights[:5], color = 'g', lw = lw, ls = '-',
        label = "post-pre pairing")
axA.set_ylabel("normalized weight change", fontsize = fs)
axA.set_xlabel("rho (Hz)", fontsize = fs)
axA.legend(bbox_to_anchor=(1.0, 0.4), loc = 'right', prop={'size':20}, fontsize = fs - 2)
axA.set_title("synaptic weight")
fig1.subplots_adjust(left = 0.05, bottom = 0.06, right = 0.96, top = 0.97, wspace = 0.2, hspace = 0.08)
