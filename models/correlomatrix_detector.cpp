...
void nest::correlomatrix_detector::State_::get(DictionaryDatum &d) const
{
  ...
  for (size_t i = 0; i < covariance_.size(); ++i)
  {
    ...
    {
      C_i->push_back( new DoubleVectorDatum( new std::vector<double_t>(covariance_[i][j]) ));
...
void nest::correlomatrix_detector::State_::set(const DictionaryDatum&, const Parameters_&, bool)
{}
...
void nest::correlomatrix_detector::init_state_(const Node& proto)
{ 
...
void nest::correlomatrix_detector::calibrate()
{
  device_.calibrate();
}
...
void nest::correlomatrix_detector::update(Time const &, const long_t, const long_t)
{}

void nest::correlomatrix_detector::handle(SpikeEvent & e)
{
  ...
  if ( device_.is_active(stamp) )
  {
    ...
    while ( !otherSpikes.empty() && (spike_i - otherSpikes.front().timestep_) >= tau_edge + min_delay)
	...
	{
	  bin = -1. * std::floor( (0.5*P_.delta_tau_.get_steps() - std::abs(spike_i - spike_j->timestep_))  / P_.delta_tau_.get_steps() );
	}
	else
	{
	  bin = std::floor( (0.5*P_.delta_tau_.get_steps() + std::abs(spike_i - spike_j->timestep_))  / P_.delta_tau_.get_steps() );
	}
	...
	if(bin < S_.covariance_[sender_ind][other_ind].size())
	{
	  ...
	  if(bin==0 && (spike_i - spike_j->timestep_ != 0 || other != sender))
	    S_.covariance_[other_ind][sender_ind][bin] += e.get_multiplicity() * e.get_weight() * spike_j->weight_;
