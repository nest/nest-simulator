...
J_syn = np.array([[J, -g * J], [J, -g * J]]) * C0 / C  # final synaptic
#                                                        weights scaling as 1/C
...
nest.SetStatus(nest_mm, {'record_from': ['n_events', 'mean'],
               'withgid': True, 'withtime': False, 'interval': dt_rec})
...
for i, nest_i in enumerate(nest_pops):
    ...
    nest.SetStatus(nest_sd[i], {'withgid': False, 'withtime': True,
                                'time_in_steps': True})
...
neurons. On the other hand, if we want to know single neuron activities, we
must simulate on the microscopic level. This is possible by building a
corresponding network of gif_psc_exp neuron models:
'''
