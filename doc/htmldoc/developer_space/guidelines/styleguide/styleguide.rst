...
We use a bib file as a single source for references: ``doc/htmldoc/refs.bib``
Any new entry has to be added to that file.

For in-text citations, we use the reStructuredText numeric style ``:footcite:p:`Smith2009```.

To generate a bibliography on a restructuredText page use:

``.. footbibliography::``

and this will generate the full citation for every ``footcite`` on that page.
