...
class mc_neuron_ode():
        ...
        def fun_u(self, t, y):
                f = [1. / self.C_m[0, 0] * (-self.G_l[0, 0]\
                     * (y[0] - self.E_l[0, 0])\
                     - y[12] * (y[0] - self.E_ex[0, 0])\
                     - y[14] * (y[0] - self.E_in[0, 0])\
                     - self.G_conn[0] * ((y[0] - self.E_l[0, 0])\
                     - (y[1] - self.E_l[1, 0])) + self.I_curr[0, 0]),\
                     1. / self.C_m[1, 0] * (-self.G_l[1, 0]\
                     * (y[1] - self.E_l[1, 0])\
                     - y[8] * (y[1] - self.E_ex[1, 0])\
                     - y[10] * (y[1] - self.E_in[1, 0])\
                     - self.G_conn[0] * ((y[1] - self.E_l[1, 0])\
                     - (y[0] - self.E_l[0, 0]))\
                     - self.G_conn[1] * ((y[1] - self.E_l[1, 0])\
                     - (y[2] - self.E_l[2, 0])) + self.I_curr[1, 0]),\
                     1. / self.C_m[2, 0] * (-self.G_l[2, 0]\
                     * (y[2] - self.E_l[2, 0])\
                     - y[4] * (y[2] - self.E_ex[2, 0])\
                     - y[6] * (y[2] - self.E_in[2, 0])\
                     - self.G_conn[1] * ((y[2] - self.E_l[2, 0])\
                     - (y[1] - self.E_l[1, 0])) + self.I_curr[2, 0]\
                     + self.catmp),\
        ...
        def __init__(self, active, resolution):
                ...
                self.Conn_mat = np.array([[self.G_conn[0],\
                                -self.G_conn[0], 0.],\
                                [-self.G_conn[0], self.G_conn[0]\
                                + self.G_conn[1] + self.E_l[1],\
                                - self.G_conn[1]], [0.,\
                                - self.G_conn[1],\
                                self.G_conn[1] + self.E_l[2]]])
                ...
                self.y_exc = np.array([[[0.], [0.]],\
                             [[0.], [0.]], [[0.], [0.]]])
                self.y_inh = np.array([[[0.], [0.]],\
                             [[0.], [0.]], [[0.], [0.]]])
                self.y_ca = np.array([[[0.], [0.]],\
                             [[0.], [0.]], [[0.], [0.]]])

                # distal_spike parameters
                self.thca = -24.5
                self.ca = np.array([0.113805061345,\
                2.57207665232,\
                18.02752317,\
                48.114851194,\
                85.6436354848,\
...
class FixedMultiCmptNeuTestCase(unittest.TestCase):
    ...
    def test_FixedMultiCmptNeu(self):
        ...
        for t in range(0, l):
                ...
                tmp_exc = amp_exc * exc_wt\
                * np.reshape(exc_input_arr[:, t], (tot_exc, 1))
                tmp_inh = amp_inh * inh_wt\
                * np.reshape(inh_input_arr[:, t], (tot_inh, 1))
        ...
        nest.Connect([sgs[0]], n, syn_spec={\
        'receptor_type': syns['distal_exc'], "weight": amp_exc, "delay": 1.0})
