...
RecordablesMap< nest::glif_lif_asc_psc >::create()
{
  ...
  insert_(
    Name( "AScurrents_sum" ), &nest::glif_lif_asc_psc::get_AScurrents_sum_ );
...
nest::glif_lif_asc_psc::Buffers_::Buffers_( const Buffers_&,
  glif_lif_asc_psc& n )
  : logger_( n )
{
}
...
void
nest::glif_lif_asc_psc::update( Time const& origin,
  const long from,
  const long to )
{
...
nest::port
nest::glif_lif_asc_psc::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type <= 0
