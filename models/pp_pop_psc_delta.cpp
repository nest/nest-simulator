...
namespace nest
{
  ...
  template <>
  void RecordablesMap<pp_pop_psc_delta>::create()
  {
    // use standard names whereever you can for consistency!
    insert_(names::V_m, &pp_pop_psc_delta::get_V_m_);
    insert_(names::n_events, &pp_pop_psc_delta::get_n_events_);  // n_events instead of E_sfa
  }
...
void nest::pp_pop_psc_delta::State_::get(DictionaryDatum &d, const Parameters_&) const
{
  ...
  int n_spikes = n_spikes_past_.size()>0 ? n_spikes_past_[p_n_spikes_past_] : 0; // return 0 if n_spikes_past_ has not been initialized yet
...
void nest::pp_pop_psc_delta::init_state_(const Node& proto)
{
  const pp_pop_psc_delta& pr = downcast<pp_pop_psc_delta>(proto);
  S_ = pr.S_;
}

void nest::pp_pop_psc_delta::init_buffers_()
{
...
void nest::pp_pop_psc_delta::update(Time const & origin, const long_t from, const long_t to)
{
 ...
 for ( long_t lag = from ; lag < to ; ++lag )
  {
    ...
    for(uint_t i = 0 ; i < V_.eta_kernel_.size() ; i++){
        tmp_vector.push_back(V_.eta_kernel_[i] * S_.n_spikes_past_[(S_.p_n_spikes_past_ + i) % S_.n_spikes_past_.size()]* V_.h_ * 0.001);
    ...
    S_.p_n_spikes_past_ = (S_.p_n_spikes_past_ - 1 + S_.n_spikes_past_.size()) % S_.n_spikes_past_.size(); // shift to the right
    ...
    int_t last_element_value = S_.age_occupations_[(S_.p_age_occupations_ - 1 + S_.age_occupations_.size()) % S_.age_occupations_.size() ]; // save the last element

    S_.p_age_occupations_ = (S_.p_age_occupations_ - 1 + S_.age_occupations_.size()) % S_.age_occupations_.size(); // shift to the right
    S_.age_occupations_[ (S_.p_age_occupations_ - 1 + S_.age_occupations_.size()) % S_.age_occupations_.size() ] += last_element_value;
   ...
   // test if S_.n_spikes_past_[S_.p_n_spikes_past_]!=0, generate spike and send this number as the parameter
...
void nest::pp_pop_psc_delta::handle(SpikeEvent & e)
{
...
void nest::pp_pop_psc_delta::handle(CurrentEvent& e)
{
  assert(e.get_delay() > 0);
...
void nest::pp_pop_psc_delta::handle(DataLoggingRequest &e)
{
  B_.logger_.handle(e);
}
