...
synindex
ConnectionManager::copy_synapse_prototype( synindex old_id,
  std::string new_name )
{
  ...
  for ( thread t = 0; t < net_.get_num_threads(); ++t )
  {
    prototypes_[ t ].push_back(
      get_synapse_prototype( old_id ).clone( new_name ) );
...
void
ConnectionManager::set_prototype_status( synindex syn_id,
...
DictionaryDatum
ConnectionManager::get_synapse_status( index gid,
  synindex syn_id,
  port p,
  thread tid )
{
  ...
  ( *dict )[ names::synapse_model ] =
    LiteralDatum( get_synapse_prototype( syn_id ).get_name() );
...
ConnectionManager::get_connections( DictionaryDatum params ) const
{
  ...
  std::string msg;
  msg = String::compose(
    "Setting OpenMP num_threads to %1.", net_.get_num_threads() );
  net_.message(
...
#pragma omp critical
      ...
      for ( index s = 0; s < source->size(); ++s )
      {
        ...
        if ( source_id < connections_[ t ].size()
          && connections_[ t ].get( source_id ) != 0 )
        {
          if ( target == 0 )
          {
            validate_pointer( connections_[ t ].get( source_id ) )
...
ConnectorBase*
ConnectionManager::validate_source_entry( thread tid,
  index s_gid,
  synindex syn_id )
{
  assert_valid_syn_id( syn_id );
  ...
  double_t w )
{
  ...
  ConnectorBase* c =
    prototypes_[ tid ][ syn ]->add_connection( s, r, conn, syn, d, w );
  ...
  double_t w )
{
  ...
  ConnectorBase* conn = validate_source_entry( tid, s_gid, syn );
  ConnectorBase* c =
    prototypes_[ tid ][ syn ]->add_connection( s, r, conn, syn, p, d, w );
  connections_[ tid ].set( s_gid, c );
}
 ...
 */
void
ConnectionManager::disconnect( Node& target,
  index sgid,
  ...
  index syn_id )
{
  ...
  if ( net_.is_local_gid( target.get_gid() ) )
  {
    ...
    ConnectorBase* conn =
      validate_pointer( validate_source_entry( target_thread, sgid, syn_id ) );
    ConnectorBase* c =
      prototypes_[ target_thread ][ syn_id ]->delete_connection(
...
{
  // #ifdef _OPENMP
  //     net_.message(SLIInterpreter::M_INFO, "ConnectionManager::Connect",
  //     msg);
  // #endif
