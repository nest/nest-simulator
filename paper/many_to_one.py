...
def find_equilibrium():
    ...
    while not (9.9 < rate < 10.1 and 0.99 < cv < 1.01):
        ...
        if rate >= 10.1:
            eta *= 1.0 - 0.03 * abs(rate - 10.0) * max(0.7, random.random())
        else:
            eta *= 1.0 + 0.02 * abs(rate - 10.0) * max(0.7, random.random())
...
def find_equilibrium_stdp():
    ...
    cv = 0
    syn_weight = 0
    while not (44.0 < syn_weight < 46.0 and 9.9 < rate < 10.1 and 0.99 < cv < 1.01):
...
def plot_corrections():
    ...
    if os.path.exists("num_corrections.npy") and os.path.exists("num_spikes.npy"):
        ...
        for i, p in enumerate(ax_perc):
            print(i)
            stdp_params["dendritic_delay"] = 5.0 * (1 - p)
            stdp_params["axonal_delay"] = 5.0 * p
...
def plot_weights():
    if os.path.exists("weights_correction.npy") and os.path.exists("weights_no_correction.npy"):
        ...
        sns.distplot(
            weights_correction, hist=False, kde=True, kde_kws={"fill": True, "linewidth": 3}, label="correction", ax=ax
        )
        sns.distplot(
        ...
        stdp_params["dendritic_delay"] = 0.0
        stdp_params["axonal_delay"] = 5.0
...
def plot_benchmark_static():
    ...
    df_master = (
        pd.read_csv("benchmark-results/hpc_axonal_delay_master_static.csv", delimiter=",")
        .drop("rng_seed", axis=1)
        .groupby("num_nodes")
        .agg("mean")
        .reset_index()
    )
    df_corr = (
        pd.read_csv("benchmark-results/hpc_axonal_delay_correction_static.csv", delimiter=",")
        .drop("rng_seed", axis=1)
        .groupby("num_nodes")
        .agg("mean")
        .reset_index()
    ...
    df_adj = (
        pd.read_csv("benchmark-results/hpc_axonal_delay_adjacency_static.csv", delimiter=",")
        .drop("rng_seed", axis=1)
        .groupby("num_nodes")
        .agg("mean")
        .reset_index()
    ...
    plt.plot(
        df_corr["num_nodes"], df_corr["py_time_simulate"], label="corr", marker="*", ms=8, linewidth=2, color=cmap(0)
    )
...
def plot_benchmark_stdp():
    ...
    df_corr = pd.read_csv("benchmark-results/hpc_axonal_delay_correction_stdp.csv", delimiter=",").drop(
        "rng_seed", axis=1
    )
    df_adj = pd.read_csv("benchmark-results/hpc_axonal_delay_adjacency_stdp.csv", delimiter=",").drop(
        "rng_seed", axis=1
    )
    df_corr_1 = (
        df_corr[df_corr["num_nodes"] == 1].groupby(["axonal_delay", "dendritic_delay"]).agg("mean").reset_index()
    )
    df_corr_32 = (
        df_corr[df_corr["num_nodes"] == 32].groupby(["axonal_delay", "dendritic_delay"]).agg("mean").reset_index()
    )
    ...
    axs[0].plot(
        df_corr_1["axonal_delay"] / (df_corr_1["axonal_delay"] + df_corr_1["dendritic_delay"]),
        df_corr_1["py_time_simulate"],
        label="corr 1",
        marker="*",
        ms=8,
        linewidth=2,
    ...
    axs[1].plot(
        df_corr_32["axonal_delay"] / (df_corr_32["axonal_delay"] + df_corr_32["dendritic_delay"]),
        df_corr_32["py_time_simulate"],
        label="corr 32",
        marker="*",
        ms=8,
        linewidth=2,
    ...
    axs[0].plot(
        df_adj_1["axonal_delay"] / (df_adj_1["axonal_delay"] + df_adj_1["dendritic_delay"]),
        df_adj_1["py_time_simulate"],
        label="adj 1",
        marker="X",
        ms=6,
        linewidth=2,
    ...
    axs[1].plot(
        df_adj_32["axonal_delay"] / (df_adj_32["axonal_delay"] + df_adj_32["dendritic_delay"]),
        df_adj_32["py_time_simulate"],
        label="adj 32",
        marker="X",
        ms=6,
        linewidth=2,
