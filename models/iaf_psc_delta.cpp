...
nest::iaf_psc_delta::Parameters_::get( dictionary& d ) const
{
  ...
  d[ names::V_th.toString() ] = V_th_ + E_L_; // threshold value
  d[ names::V_reset.toString() ] = V_reset_ + E_L_;
  d[ names::V_min.toString() ] = V_min_ + E_L_;
...
nest::iaf_psc_delta::Parameters_::set( const dictionary& d, Node* node )
{
  ...
  if ( update_value_param( d, names::V_th.toString(), V_th_, node ) )
  {
    V_th_ -= E_L_;
  }
  else
  ...
  if ( update_value_param( d, names::V_min.toString(), V_min_, node ) )
  {
    V_min_ -= E_L_;
  }
  else
...
{
  if ( update_value_param( d, names::V_m.toString(), y3_, node ) )
  {
    y3_ -= p.E_L_;
  }
  else
