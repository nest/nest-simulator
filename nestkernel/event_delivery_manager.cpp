...
EventDeliveryManager::configure_spike_buffers()
{
  ...
  send_recv_count_target_data_per_rank_ = floor( kernel().mpi_manager.get_buffer_size_target_data() / kernel().mpi_manager.get_num_processes() );
  send_recv_count_target_data_in_int_per_rank_ = sizeof( TargetData ) / sizeof( unsigned int ) * send_recv_count_target_data_per_rank_;

  send_buffer_target_data_ = static_cast< TargetData* >( malloc( kernel().mpi_manager.get_buffer_size_target_data() * sizeof( TargetData ) ) );
  recv_buffer_target_data_ = static_cast< TargetData* >( malloc( kernel().mpi_manager.get_buffer_size_target_data() * sizeof( TargetData ) ) );
...
#pragma omp single
    ...
    me_completed_tid = collocate_target_data_buffers_( tid, send_recv_count_target_data_per_rank_, send_buffer_target_data_ );
#pragma omp atomic
    completed_count += me_completed_tid;
#pragma omp barrier
    ...
    if ( completed_count == half_completed_count )
    {
      set_complete_marker_target_data_( tid, send_recv_count_target_data_per_rank_, send_buffer_target_data_ );
#pragma omp barrier
    }
...
#pragma omp single
    {
      ...
      kernel().mpi_manager.communicate_Alltoall( send_buffer_int, recv_buffer_int, send_recv_count_target_data_in_int_per_rank_ );
      sw_communicate_target_data.stop();
    } // of omp single

    sw_deliver_target_data.start();
    others_completed_tid = distribute_target_data_buffers_( tid, send_recv_count_target_data_per_rank_, recv_buffer_target_data_ );
