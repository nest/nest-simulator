...
namespace nest
{
  ...
  inline
  port aeif_cond_alpha::send_test_event(Node& target, rport receptor_type, synindex, bool)
  {
    SpikeEvent e;
    e.set_sender(*this);
  ...
  inline
  port aeif_cond_alpha::handles_test_event(SpikeEvent&, rport receptor_type)
  {
    if (receptor_type != 0)
  ...
  inline
  port aeif_cond_alpha::handles_test_event(CurrentEvent&, rport receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
    return 0;
  ...
  inline
  port aeif_cond_alpha::handles_test_event(DataLoggingRequest &dlr, 
                                           rport receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
