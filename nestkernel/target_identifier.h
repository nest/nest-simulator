...
public:
  ...
  Node *get_target_ptr(thread) const { return target_; }
...
public:
  ...
  {
    if (rprt != 0)
      throw IllegalConnection("Only rport==0 allowed for HPC synpases. Use normal synapse models instead."
...
void TargetIdentifierIndex::set_target(Node *target)
{
  ...
  if (target_lid > max_targetindex)
    throw IllegalConnection(String::compose("HPC synapses support at most %1 nodes per thread. "
    						                "See Kunkel et al, Front Neuroinform 8:78 (2014), Sec 3.3.2.",
    		                                max_targetindex)
    			           );
  target_ = target_lid;
