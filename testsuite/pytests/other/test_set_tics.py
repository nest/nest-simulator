...
per computation time step can be changed at run time. This is of advantage
...
data type. These parameters can be set at run time in the NEST kernel prior
to the creation of any network element. Here, it is often more convenient to
specify the resolution (the computation time step) in units of milliseconds
instead of the number of tics per step.
