...
The Topology Module provides the NEST simulator\[1\]
\[[1](#ref-Gewa:2007)\] with a convenient interface for creating layers
of neurons placed in space and connecting neurons in such layers with
probabilities and properties depending on the relative placement of
neurons. This permits the creation of complex networks with spatial
structure.
...
Python interface to NEST \[[2](#ref-Eppl:2008)\]. NEST users using the
SLI interface should be able to map instructions to corresponding SLI
code. This manual is not meant as a comprehensive reference manual.
Please consult the online documentation in PyNEST for details; where
...
place neurons in space. In Chapter [3](#sec:connections) we then
describe how to connect layers with each other, before discussing in
Chapter [4](#sec:inspection) how you can inspect and visualize Topology
networks. Chapter [5](#ch:extending) deals with the more advanced topic
of extending the Topology module with custom kernel functions and masks
...
![Figure 1: Simple grid-based layer centered about the origin. Blue
circles mark layer elements, the thin square the extent of the layer.
Row and column indices are shown in the right and top margins,
respectively.](user_manual_figures/layer1.png)

The layer is shown in Fig. [1](#fig:layer1). Note the following
properties:
  ...
  - The *extent* or size of the layer is \(1\times  1\). This is the
    default size for layers. The extent is marked by the thin square in
    Fig. [1](#fig:layer1).

  - The *grid spacing* of the layer is \[\begin{split}
    dx &= \frac{\text{x-extent}}{\text{number of columns}} \\
    dy &= \frac{\text{y-extent}}{\text{number of rows}}
    \end{split}\qquad (1)\] In the layer shown, we have \(dx=dy=0.2\),
  ...
  - Each element of a grid-based layer has a *row- and column-index* in
    ...
    and right margin of Fig. [1](#fig:layer1). Note that row-indices
...
\(y\)-direction by adding an `'extent'` entry to the dictionary passed
to `CreateLayer`:
...
![Figure 2: Same layer as in Fig. [1](#fig:layer1), but with different
extent.](user_manual_figures/layer2.png)

The resulting layer is shown in Fig. [2](#fig:layer2). The extent is
...
The size of `'extent'` in \(x\)- and \(y\)-directions should be numbers
that can be expressed exactly as binary fractions. This is automatically
...
![Figure 3: Three layers centered, respectively, about \((0,0)\) (blue),
\((-1,-1)\) (green), and \((1.5,0.5)\)
(red).](user_manual_figures/layer3.png)
...
Fig. [3](#fig:layer3), grid indices run from 0 to 4 through columns and
...
From Eq. [1](#eq:dx_dy_extent), we see that the extent of the layer must
be \((n_c d, n_r d)\). We now need to find the coordinates
\((c_x, c_y)\) of the center of the layer. To place the left edge of the
extent at \(x=0\), we must place the center of the layer at
\(c_x=n_c d / 2\) along the \(x\)-axis, i.e., half the extent width to
the right of \(x=0\). Since the layer is to be centered about \(y=0\),
we have \(c_y=0\). Thus, the center coordinates are \((n_c d/2, 0)\).
The layer is created with the following code and shown in Fig.
[4](#fig:layer3a):
...
![Figure 4: Layer with \(n_c=5\) rows and \(n_r=3\) columns, spacing
\(d=0.1\) and the left edge of the extent at \(x=0\), centered about the
\(y\)-axis. The cross marks the point on the extent placed at the origin
\((0,0)\), the circle the center of the
...
![Figure 5: A free layer with 50 elements uniformly distributed in an
extent of size \(1\times 1\).](user_manual_figures/layer4.png)
...
![Figure 6: A free 3D layer with 200 elements uniformly distributed in
an extent of size
...
network. In the \(5\times 5\)-layer in Fig. [1](#fig:layer1), e.g., 16
...
surface of a torus. Fig. [7](#fig:player) illustrates this for a
...
![Figure 7: Top left: Layer with single row and five columns without
...
distance from the element at \((-2,0)\). Bottom left: Same layer, but
with periodic boundary conditions. Note that the element at \((2,0)\)
now is a nearest neighbor to the element at \((-2,0)\). Right: Layer
with periodic boundary condition arranged on a circle to illustrate
neighborhood relationships.](user_manual_figures/player.png)
...
interest:
  ...
  - The status dictionary of a layer has a `'topology'` entry describing
    the layer properties (`l` is the layer created
above):
...
Note the following points:
  ...
  - For grid-based layers, the function `GetElement` returns a list of
    nodes at a given grid position. See Chapter [4](#sec:inspection) for
...
> The network consists of \(20x20\) microcolumns placed on a regular
> grid spanning \(0.5^\circ\times 0.5^\circ\) of visual space. Neurons
> within each microcolumn are organized into L2/3, L4, and L56
...
We will discuss in Chapter [3.1](#sec:conn_basics) how to connect
...
We begin by introducing important terminology:
  ...
  - Mask  
    ...
    Sec. [3.3](#sec:conn_masks) for details.
  ...
  - Kernel  
    ...
    Sec. [3.4](#sec:conn_kernels) for details.
...
In many cases, the connection dictionary will also contain
  ...
  - ‘mask’  
    a mask specification as described in Sec. [3.3](#sec:conn_masks).
...
Here is a simple example, cf. [8](#fig:conn1)
...
![Figure 8: Left: Minimal connection example from a layer onto itself
using a rectangular mask shown as red line for the node at \((0,0)\)
(marked light red). The targets of this node are marked with red dots.
The targets for the node at \((4,5)\) are marked with yellow dots. This
node has fewer targets since it is at the corner and many potential
targets are beyond the layer. Right: The effect of periodic boundary
conditions is seen here. Source and target layer and connection
dictionary were identical, except that periodic boundary conditions were
...
they would form a \(5\times 3\) rectangle centered on the node at
\((4,5)\).](user_manual_figures/conn1.png)
...
3.  The displacement \(d(D,P)\) from node \(D\) in the driver layer to
    node \(P\) in the pool layer is measured by first mapping the
    position of \(D\) in the driver layer to the identical position in
    the pool layer and then computing the displacement from that
    position to \(P\). If the pool layer has periodic boundary
    conditions, they are taken into account. It does not matter for
    displacement computations whether the driver layer has periodic
    boundary conditions.
...
covered in Chapter [5](#ch:extending).
...
Fig. [9](#fig:conn2_a). The masks are
...
![Figure 9: Masks for 2D layers. For all mask types, the driver node is
marked by a wide light-red circle, the selected pool nodes by red dots
and the masks by red lines. From left to right, top to bottom:
rectangular, circular, doughnut and elliptical masks centered about the
driver node.](user_manual_figures/conn2_a.png)
...
examples (cf. Fig. [10](#fig:conn2_b)).
...
![Figure 10: The same masks as in Fig. [9](#fig:conn2_a), but centered
about \((-1.5,-1.5)\), \((-2,0)\), \((1.5,1.5)\) and \((2, -1)\),
respectively, using the `'anchor'`
parameter.](user_manual_figures/conn2_b.png)

and \(\textbf{elliptical}\) masks, see Fig [10](#fig:conn2_b). To do so,
add an `'azimuth_angle'` entry in the specific mask dictionary. The
`azimuth_angle` is measured in degrees and is the rotational angle from
...
![Figure 11: Rotated rectangle and elliptical mask from
Fig. [9](#fig:conn2_a) and Fig. [10](#fig:conn2_b), where the rectangle
mask is rotated \(120^\circ\) and the elliptical mask is rotated
\(45^\circ\).](user_manual_figures/conn2_c.png)
...
![Figure 12: Masks for 3D layers. For all mask types, the driver node is
marked by a wide light-red circle, the selected pool nodes by red dots
and the masks by red lines. From left to right: box and spherical masks
centered about the driver node.](user_manual_figures/conn_3d.png)
...
The resulting connections are shown in Fig. [13](#fig:conn3). By default
the top-left corner of a grid mask, i.e., the grid mask element with
grid index \([0,0]\)\[4\], is aligned with the driver node. You can
...
The resulting connection patterns are shown in Fig. [13](#fig:conn3).

![Figure 13: Grid masks for connections between grid-based layers. Left:
...
\([-1,2]\).](user_manual_figures/conn3.png)
...
Note the following:
  ...
  - The semantics of the `'anchor'` property for grid-based masks differ
    ...
    Sec. [3.3.1](#sec:free_masks). For general masks, the anchor is the
...
Topology:
  ...
  - Free probabilistic connections  
    ...
    pool node \(P\) within the mask and creates a connection according
    to the resulting probability. This means in particular that *each
  ...
  - Prescribed number of connections  
    ...
    per driver node. See Sec. [3.7](#sec:prescribed_numbers) for
    details.

Available kernel functions are shown in Table [1](#tbl:kernels). More
kernel functions may be created in a NEST extension module. This is
covered in
Chapter [5](#ch:extending).
...
| `gaussian`    | `p_center`, `sigma`, `mean`, `c`                                | \[p(d) = c + p_{\text{center}}  e^{-\frac{(d-\mu)^2}{2\sigma^2}}\]                                                                                                                |
| `gaussian2D`  | `p_center`, `sigma_x`, `sigma_y`, `mean_x`, `mean_y`,`rho`, `c` | \[p(d) = c + p_{\text{center}}
...
| `normal`      | `mean`, `sigma`, `min`, `max`                                   | \(p \in [\text{min},\text{max})\) normal with given mean and \(\sigma\)                                                                                                           |
| `lognormal`   | `mu`, `sigma`, `min`, `max`                                     | \(p \in [\text{min},\text{max})\) lognormal with given \(\mu\) and \(\sigma\)                                                                                                     |

Table 1: Functions currently available in the Topology module. \(d\) is
the distance and \((d_x,d_y)\) the displacement. All functions can be
used to specify weights and delays, but only the constant and the
distance-dependent functions, i.e., all functions above the double line,
can be used as kernels. 

![Figure 14: Illustration of various kernel functions. Top left:
constant kernel, \(p=0.5\). Top center: Gaussian kernel, green dashed
lines show \(\sigma\), \(2\sigma\), \(3\sigma\). Top right: Same
Gaussian kernel anchored at \((1.5,1.5)\). Bottom left: Same Gaussian
kernel, but all \(p<0.5\) treated as \(p=0\). Bottom center:
2D-Gaussian.](user_manual_figures/conn4.png)

Several examples follow. They are illustrated in Fig. [14](#fig:conn4).
...
```
  ...
  - Gaussian  
    ...
    probability is 1 for \(d=0\) and falls off with a “standard
...
```
  ...
  - Cut-off Gaussian  
    In this example, all probabilities less than \(0.5\) are set to
    zero:
...
```
  ...
  - 2D Gaussian  
    ...
    a Gaussian with different widths in \(x\)- and \(y-\) directions.
    This kernel depends on displacement, not only on distance:
...
The functions presented in Table [1](#tbl:kernels) can also be used to
specify distance-dependent or randomized weights and delays for the
connections created by `ConnectLayers`.

Figure [15](#fig:conn5) illustrates weights and delays generated using
...
Results are shown in the top panel of Fig. [15](#fig:conn5). Connection
weights and delays are shown for the leftmost neuron as driver. Weights
drop linearly from \(1\). From the node at \((20,0)\) on, the cutoff
sets weights to 0. There are no connections to nodes beyond \((25,0)\),
since the mask extends only 25 units to the right of the driver. Delays
increase in a stepwise linear fashion, as NEST requires delays to be
multiples of the simulation resolution.
...
Results are shown in the middle panel of Fig. [15](#fig:conn5). This
example is identical to the previous, except that the (pool) layer has
periodic boundary conditions. Therefore, the left half of the mask about
the node at \((0,0)\) wraps back to the right half of the layer and that
node connects to all nodes in the layer.
...
Results are shown in the bottom panel of Fig. [15](#fig:conn5). It shows
...
By using the `'uniform'` function for weights or delays, one can obtain
randomized values for weights and delays, as shown by the red circles in
the bottom panel of Fig. [15](#fig:conn5). Weights and delays can
currently only be randomized with uniform distribution.

![Figure 15: Distance-dependent and randomized weights and delays. See
text for details.](user_manual_figures/conn5.png)
...
4.  If you prohibit multapses (cf Sec. [3.1.1](#sec:terminology)) and
...
\[p_{\text{conn}}(d) = \frac{12}{\pi} \times 2\pi r \times (1-2r)
 = 24 r (1-2r) \qquad \text{for} \quad 0\le r < \frac{1}{2}\;.\qquad (2)\]
 The resulting distribution of distances between connected nodes is
shown in Fig. [16](#fig:conn6).
...
![Figure 16: Distribution of distances between source and target for a
network of 1000 randomly placed nodes, a fixed fan out of 50 connections
and a connection probability decaying linearly from 1 to 0 at \(d=0.5\).
The red line is the expected distribution from
Eq. [2](#eq:ptheo).](user_manual_figures/conn6.png)
...
Sec. [3.10](#sec:dev_subregions), such as spike detectors, are only
...
functions, please see the online Python and SLI
documentation.
...
| `tp.SelectNodesByMask()`    | Obtain GIDs of nodes/elements inside a masked area of a layer. Part of NEST since NEST 2.14.                                                               |

## Visualization functions

Topology provides three functions to visualize
networks:
...
![Figure 17: \(21\times 21\) grid with divergent Gaussian projections
...
large red circle is the mask, the dashed green lines mark \(\sigma\),
\(2\sigma\) and \(3\sigma\) of the Gaussian
kernel.](user_manual_figures/vislayer.png)
...
resulting graphics is shown in Fig. [17](#fig:vislayer). All elements
...
Module”](http://nest.github.io/nest-simulator/extension_modules) in
...
parameters \(a\), \(b\) and \(c\) from the dictionary provided by the
user. The function `updateValue` will do nothing if the given key is not
in the dictionary, and the default values \(a=b=1,\ c=0\) will be used.
...
Sec. [3.3](#sec:conn_masks). That elliptical mask is defined in a
...
Module from NEST version 2.0 to 2.2.
  ...
  - It is now possible to add kernel functions and masks to the Topology
    ...
    Chapter [5](#ch:extending) for examples.
...
Topology Module from the 1.9-xxxx to the 2.0 version.
  ...
  - Several other functions changed names, and there are many new
    functions. Please see Ch. [4](#sec:inspection) for an overview.
  ...
  - The semantics of the `anchor` entry for kernel functions has
    ...
    the semantics for free masks, see Sec. [3.3](#sec:conn_masks) and
    [3.4](#sec:conn_kernels).
...
1\. Gewaltig M-O, Diesmann M (2007). NEST (neural simulation tool).
...
2\. Eppler JM, Helias M, Muller E et al. (2008). PyNEST: A convenient
interface to the NEST simulator. Front Neuroinformatics 2 12. doi:
[10.3389/neuro.11.012.2008](https://doi.org/10.3389/neuro.11.012.2008).

</div>

<div id="ref-Nord:2009">

3\. Nordlie E, Gewaltig M-O, Plesser HE (2009). Towards reproducible
descriptions of neuronal network models. PLoS Comput Biol 5 (8):
e1000456. doi:
[10.1371/journal.pcbi.1000456](https://doi.org/10.1371/journal.pcbi.1000456).
...
2.  See \[[3](#ref-Nord:2009)\](456) for suggestions on how to describe
    network models.

3.  You can also use standard NEST connection functions to connect nodes
    in Topology layers.

4.  See Sec. [2.1.1](#sec:verysimple) for the distinction between layer
