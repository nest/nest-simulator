...
class TestBernoulliSynapse:
    ...
    def test_bernoulli_synapse_statistics(seed: int):
        r"""Measure average number of spikes transmitted by a Bernoulli synapse (static synapse with stochastic transmission).

        1000 spikes generated by spike_generator are sent to a parrot neuron using a bernoulli_synapse with transmission probability 25%. Transmission is mediated by another parrot neuron that is connected to spike_generator via static synapse. If the average number of spikes detected by spike recorders is within three standard deviations from the mean of the binomial distribution, the synapse works fine."""
        nest.ResetKernel()
        nest.SetKernelStatus({"rng_seed": seed})

        # test parameters
        N_spikes = 1000    # number of spikes sent via bernoulli synapse to parrot neuron
        p = 0.25    # transmission probability of the bernoulli synapse
        margin = 3 * (N_spikes * p * (1 - p))**.5    # allowed absolute deviation: three standard deviations of the binomial distribution with (N_spikes, p)
        ...
        # connect presynaptic parrot neuron to postsynaptic parrot neuron via Bernoulli_synapse with transmission probability p
