...
namespace nest{
  ...
  /* BeginDocumentation
    Name: gif_psc_exp_multisynapse - Current based generalized integrate-and-fire neuron model according to Mensi et al. (2012) 
    and Pozzorini et al. (2015)

    Description:

    gif_psc_exp_multisynapse is the generalized integrate-and-fire neuron according to Mensi et al. (2012)
    ...
    Synaptic parameters
      taus_syn  vector of double - Time constants of the synaptic currents in ms (exp function).
  ...
  inline
port gif_psc_exp_multisynapse::send_test_event(Node& target, rport receptor_type, synindex, bool)
{
  SpikeEvent e;
  e.set_sender(*this);
...
inline
port gif_psc_exp_multisynapse::handles_test_event(CurrentEvent&, rport receptor_type)
{
  if (receptor_type != 0)
...
inline
port gif_psc_exp_multisynapse::handles_test_event(DataLoggingRequest &dlr, 
				    rport receptor_type)
{
  if (receptor_type != 0)
    throw UnknownReceptorType(receptor_type, get_name());
...
inline
void gif_psc_exp_multisynapse::get_status(DictionaryDatum &d) const
{
  P_.get(d);
  S_.get(d, P_);
...
inline
void gif_psc_exp_multisynapse::set_status(const DictionaryDatum &d)
{
