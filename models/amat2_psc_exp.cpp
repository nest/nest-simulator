...
namespace nest   // template specialization must be placed in namespace
{
  ...
  void nest::amat2_psc_exp::Parameters_::get(DictionaryDatum &d) const
  {
  ...
  double nest::amat2_psc_exp::Parameters_::set(const DictionaryDatum& d)
  {
    ...
    if ( Tau_ <= 0 || tau_ex_ <= 0 || tau_in_ <= 0 || 
         tau_ref_ <= 0 || tau_1_ <= 0 || tau_2_ <= 0 || tau_v_ <= 0)
      throw BadProperty("All time constants must be strictly positive.");

    if ( Tau_ == tau_ex_ || Tau_ == tau_in_ || Tau_ == tau_v_ )
      throw BadProperty("tau_m must differ from tau_syn_ex, tau_syn_in and tau_v. "
			            "See note in documentation.");

    if ( tau_v_ == tau_ex_ || tau_v_ == tau_in_ ) // tau_v_ == tau_m_  checked above
      throw BadProperty("tau_v must differ from tau_syn_ex, tau_syn_in and tau_m. "
			            "See note in documentation.");

    return delta_EL;
  }

  void nest::amat2_psc_exp::State_::get(DictionaryDatum &d, const Parameters_& p) const
  {
    def<double>(d, names::V_m, V_m_ + p.U0_); // Membrane potential
    def<double>(d, names::V_th, p.U0_ + p.omega_ + V_th_1_ + V_th_2_ + V_th_v_); // Adaptive threshold
    def<double>(d, names::V_th_alpha_1, V_th_1_);
    def<double>(d, names::V_th_alpha_2, V_th_2_);
    def<double>(d, names::V_th_v, V_th_v_);
  }

  void nest::amat2_psc_exp::State_::set(const DictionaryDatum& d, const Parameters_& p, double delta_EL)
  {
  ...
  void nest::amat2_psc_exp::init_state_(const Node& proto)
  {
    const amat2_psc_exp& pr = downcast<amat2_psc_exp>(proto);
    S_ = pr.S_;
  }

  void nest::amat2_psc_exp::init_buffers_()
  {
  ...
  void nest::amat2_psc_exp::calibrate()
  {
    ...
    V_.P32_ = ((eI - em)*tauI*taum)/(c*(tauI - taum));
		...
		V_.P61_ = (beta*tauE*taum*tauV*(eV*(-tauE + taum) + em*(tauE - tauV) + eE*(-taum + tauV)))/(c*(tauE - taum)*(tauE - tauV)*(taum - tauV));
		V_.P62_ = (beta*tauI*taum*tauV*(eV*(-tauI + taum) + em*(tauI - tauV) + eI*(-taum + tauV)))/(c*(tauI - taum)*(tauI - tauV)*(taum - tauV));
		V_.P63_ = (beta*(-em + eV)*tauV)/(taum - tauV);

		V_.P70_ = (beta*taum*tauV*(em*taum*tauV - eV*(h*(taum - tauV) + taum*tauV)))/(c*std::pow(taum - tauV,2));
		V_.P71_ = (beta*tauE*taum*tauV*((em*taum*std::pow(tauE - tauV,2) - eE*tauE*std::pow(taum - tauV,2))*tauV
							- eV*(tauE - taum)*(h*(tauE - tauV)*(taum - tauV) + tauE*taum*tauV
							- std::pow(tauV,3))))/(c*(tauE - taum)*std::pow(tauE - tauV,2)*std::pow(taum - tauV,2));
		V_.P72_ = (beta*tauI*taum*tauV*((em*taum*std::pow(tauI - tauV,2) - eI*tauI*std::pow(taum - tauV,2))*tauV
							- eV*(tauI - taum)*(h*(tauI - tauV)*(taum - tauV) + tauI*taum*tauV
							- std::pow(tauV,3))))/(c*(tauI - taum)*std::pow(tauI - tauV,2)*std::pow(taum - tauV,2));
		V_.P73_ = (beta*tauV*(-(em*taum*tauV) + eV*(h*(taum - tauV) + taum*tauV)))/std::pow(taum - tauV,2);
    V_.P76_ = eV*h;
  ...
  void nest::amat2_psc_exp::update(Time const & origin, const long_t from, const long_t to)
  {
  ...
  void nest::amat2_psc_exp::handle(SpikeEvent & e)
  {
  ...
  void nest::amat2_psc_exp::handle(CurrentEvent& e)
  {
    assert(e.get_delay() > 0);
  ...
  void nest::amat2_psc_exp::handle(DataLoggingRequest& e)
  {
    B_.logger_.handle(e);
  }
