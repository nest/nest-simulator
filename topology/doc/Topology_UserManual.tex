...
\item[Elliptical\index{elliptical}] All nodes within an ellipsis are connected.
  The area is specified by its major and minor axis.
...
\noindent It is possible to rotate the $\textbf{rectangular}$ and
$\textbf{elliptical}$ masks, see Fig~\ref{fig:conn2_c}. To do so, add an
\lstinline!'azimuth_angle'!\index{azimuth\_angle} entry in the
specific mask dictionary. The azimuth\_angle is measured in degrees and is the
angle from the x-axis to the center of the mask after rotation. 
...
\item[Ellipsoidal\index{ellipsoidal}] All nodes within an ellipsoid are
  connected. The area is specified by its major, minor, and polar axis.
...
or an \lstinline!'polar_angle'!\index{polar\_angle} entry, specifying the
rotation angle in degrees from the z-axis. You can specify both at once of
course. NEST currently do not support rotation in all three directions.
