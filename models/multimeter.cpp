...
Multimeter::update( Time const& origin, const long_t from, const long_t )
{
  ...
  // informs
  // handle() that the first incoming DataLoggingReply is for a new time slice,
  // so that
  // the data from that first Reply must be pushed back; all following Reply
  // data is
  // then added.
  //
...
Multimeter::handle( DataLoggingReply& reply )
{
  ...
  size_t inactive_skipped =
    0; // count records that have been skipped during inactivity
  ...
  V_.new_request_ =
    false; // correct either we are done with the first reply or any later one
}
