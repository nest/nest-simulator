...
def get_weight(PSP_val):
    ...
    PSC_e_over_PSP_e = (((C_m)**(-1)*tau_m*tau_syn_ex/(tau_syn_ex - tau_m)*((tau_m/tau_syn_ex)**(- tau_m/(tau_m-tau_syn_ex))-(tau_m/tau_syn_ex)**(- tau_syn_ex/(tau_m-tau_syn_ex))))**(-1))
...
def get_total_number_of_synapses():
    ...
    The first index (rows) of the matrix is the target population and the second (columns)
    ...
    Returns: Matrix with total number of synapses of dimension (len(populations), len(populations)).
    """
    ...
    # written in sli
    K = (((n_syn_temp * (N_full_matrix * scaling).astype(int))/N_full_matrix).astype(int))
    return K


def synapses_th_matrix():
    """This function returns the total number of synapses, between the thalamus and all target populations.
...
def adjust_w_and_ext_to_K_new(K_full, K_scaling, w, w_from_PSP, DC):
    """With this funtion the recurrent and external weights are adjusted to the scaling of the indegrees.
    Extra DC input is created to compensate the scaling.
    This is done to preserve the mean and variance of the inputs to the neurons.

    Parameters: Time constant of the external postsynaptic excitatory current (tau_syn_E),
                mean rates of the different populations in the non-scaled version (full_mean_rates),
    ...
    if net_dict['poisson_input']:
        ...
        I_ext = 0.001 * tau_syn_E * ((1. - np.sqrt(internal_scaling)) * x1_sum + (1. - np.sqrt(external_scaling)) * x1_ext) + DC
    else:
        w_ext_new = np.nan
        I_ext = 0.001 * tau_syn_E * ((1. - np.sqrt(internal_scaling)) * x1_sum) + DC
