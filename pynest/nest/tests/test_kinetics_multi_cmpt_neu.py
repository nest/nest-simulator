...
class mc_neuron_ode:
    ...
    def fun_u(self, t, y):
        f = [
            ...
            (1. / (1. + np.exp((y[2] - self.halfm) * -1. *
             self.slopem)) - y[15]) / self.taum,
            (1. / (1. + np.exp((y[2] - self.halfh) * -1. *
             self.slopeh)) - y[16]) / self.tauh,
            ]
    ...
    def __init__(self, active, resolution):
        ...
        self.slopem = 0.5
        self.slopeh = -0.5
        self.u[15] = 1. / (1. + np.exp((self.u[2] - self.halfm) * -1. *
                           self.slopem))
        self.u[16] = 1. / (1. + np.exp((self.u[2] - self.halfh) * -1. *
                           self.slopeh))
...
class KineticsMultiCmptNeuTestCase(unittest.TestCase):
    ...
    def test_KineticsMultiCmptNeu(self):
        ...
        for t in range(0, l):
            ...
            ca_arr[t] = neur.u[15] * neur.u[16] * neur.gca * (neur.eca -
                                                              neur.u[2])
        ...
        syns = nest.GetDefaults('iaf_cond_alpha_mc_kinetics'
                                )['receptor_types']
        ...
        rqs = nest.GetDefaults('iaf_cond_alpha_mc_kinetics'
                               )['recordables']
