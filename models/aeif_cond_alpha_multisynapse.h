...
inline port aeif_cond_alpha_multisynapse::handles_test_event(CurrentEvent&,
		rport receptor_type) {
	if (receptor_type != 0)
...
inline port aeif_cond_alpha_multisynapse::handles_test_event(
...
inline void aeif_cond_alpha_multisynapse::set_status(const DictionaryDatum& d) {
 ...
 */
inline void aeif_cond_alpha_multisynapse::aeif_cond_alpha_multisynapse_dynamics(
		const std::vector<double_t>& y, std::vector<double_t>& f) {
	// a shorthand
	...
	for (size_t i = 0;
			i
					< (P_.num_of_receptors_
							* S::NUMBER_OF_STATES_ELEMENTS_PER_RECEPTOR); i +=
	...
	const double_t I_spike = P_.Delta_T
			* std::exp(std::min(exp_arg, MAX_EXP_ARG));
