...
class TestFixedTotalNumber(connect_test_base.ConnectTestBase):
    ...
    def testErrorMessages(self):
        got_error = False
        conn_params = nest.FixedTotalNumber(
            source=None, target=None, N=self.N1 * self.N2 + 1, allow_autapses=True, allow_multapses=False
        )
    ...
    def testStatistics(self):
        conn_params = nest.FixedTotalNumber(
            source=None, target=None, N=self.N, allow_autapses=True, allow_multapses=True
        )
