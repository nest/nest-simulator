...
@pytest.fixture
def setup_simulation(simulation, resolution, weight, delay, syn_spec, n1_params, n2_params):
...
class TestIAFPSCAlpha1to2WithMultiRes:
    def test_1to2(self, simulate):
        _, results = simulate()
        self._utils.assert_expected_table(results, expect_default)
...
def test_mindelay_invariance(simulate, min_delay, delay, testutils):
    ...
    _, results = simulate()
    testutils.assert_expected_table(results, expect_inv)
