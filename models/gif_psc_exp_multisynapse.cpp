...
void nest::gif_psc_exp_multisynapse::Parameters_::get(DictionaryDatum &d) const
{
...
void nest::gif_psc_exp_multisynapse::Parameters_::set(const DictionaryDatum& d)
{
...
void nest::gif_psc_exp_multisynapse::State_::get(DictionaryDatum &d, const Parameters_& p) const
{
  def<double>(d, names::V_m, y3_); // Membrane potential
  def<double>(d, names::E_sfa, q_); // Adaptive threshold potential
}

void nest::gif_psc_exp_multisynapse::State_::set(const DictionaryDatum& d, const Parameters_& p)
{
...
void nest::gif_psc_exp_multisynapse::init_state_(const Node& proto)
{
...
void nest::gif_psc_exp_multisynapse::init_buffers_()
{
...
void nest::gif_psc_exp_multisynapse::calibrate()
{
...
void nest::gif_psc_exp_multisynapse::update(Time const & origin, const long_t from, const long_t to)
{
  ...
  for ( long_t lag = from ; lag < to ; ++lag )
  {
    ...
    for (size_t i = 0 ; i < P_.num_of_receptors_ ; i++)
    {
      ...
      sum_syn_pot += V_.P21_syn_[ i ] * S_.i_syn_[ i ]; // computing effect of synaptic currents on membrane potential
   ...
   {  
      ...
      if(S_.add_stc_sfa_ == true){
       ...
       S_.y3_ = V_.P30_*(S_.y0_ + P_.I_e_ - S_.stc_) + V_.P33_*S_.y3_ + V_.P31_*P_.E_L_ + sum_syn_pot;  // effect of synaptic currents (sum_syn_pot) is added here
...
port gif_psc_exp_multisynapse::handles_test_event( SpikeEvent&, rport receptor_type )
{
...
void gif_psc_exp_multisynapse::handle( SpikeEvent& e )
{
...
void nest::gif_psc_exp_multisynapse::handle(CurrentEvent& e)
{
  assert(e.get_delay() > 0);
...
void nest::gif_psc_exp_multisynapse::handle(DataLoggingRequest &e)
{
  B_.logger_.handle(e);
}
