...
RecordablesMap< nest::glif_lif_r_asc_a >::create()
{
  ...
  insert_(
    Name( "AScurrents_sum" ), &nest::glif_lif_r_asc_a::get_AScurrents_sum_ );
}
}
...
nest::glif_lif_r_asc_a::Buffers_::Buffers_( const Buffers_&,
  glif_lif_r_asc_a& n )
  : logger_( n )
{
}
...
void
nest::glif_lif_r_asc_a::update( Time const& origin,
  const long from,
  const long to )
{
