...
class CompartmentsTestCase(unittest.TestCase):
    ...
    def test_attenuation(self, model_name="1dend_1comp", dt=0.1, i_amp=1.0, t_max=2000.0):
        ...
        for ii in range(n_comp):
            ...
            nest.Connect(
                nest.AllToAll(
                    source=nest.Create("dc_generator", {"amplitude": i_amp}),
                    target=n_neat,
                    syn_spec=nest.synapsemodels.static(weight=1.0, delay=t_max / 2.0, receptor_type=ii),
                )
            )
    ...
    def test_spike_transmission(self, dt=0.01):
        ...
        nest.Connect(
            nest.AllToAll(dc, n_neat_0, syn_spec={"synapse_model": "static_synapse", "weight": 1.0, "receptor_type": 0})
        )
    ...
    def test_setstatus_combinations(self, dt=0.1):
        ...
        nest.Connect(
            nest.AllToAll(
                sg_01, n_neat_0, syn_spec={"synapse_model": "static_synapse", "weight": 0.1, "receptor_type": 0}
            )
        )
        nest.Connect(
            nest.AllToAll(
                sg_02, n_neat_0, syn_spec={"synapse_model": "static_synapse", "weight": 0.1, "receptor_type": 1}
            )
        )
        ...
        nest.Connect(
            nest.AllToAll(
                sg_11, n_neat_1, syn_spec={"synapse_model": "static_synapse", "weight": 0.1, "receptor_type": 0}
            )
        )
        nest.Connect(
            nest.AllToAll(
                sg_12, n_neat_1, syn_spec={"synapse_model": "static_synapse", "weight": 0.1, "receptor_type": 1}
            )
        )
    ...
    def test_error_handling(self):
        ...
        with self.assertRaisesRegex(
            ...
            nest.Connect(
                nest.AllToAll(
                    dc, n_neat, syn_spec={"synapse_model": "static_synapse", "weight": 1.0, "receptor_type": 3}
                )
            )
            nest.BuildNetwork()
        ...
        with self.assertRaisesRegex(
            ...
            nest.Connect(nest.AllToAll(sg, n_neat, syn_spec=nest.synapsemodels.static(weight=1.0, receptor_type=3)))
            nest.BuildNetwork()
    ...
    def test_continuerun(self, dt=0.1):
        ...
        nest.Connect(
            nest.AllToAll(sg_1, n_neat, syn_spec={"synapse_model": "static_synapse", "weight": 0.1, "receptor_type": 0})
        )
        nest.Connect(
            nest.AllToAll(sg_2, n_neat, syn_spec={"synapse_model": "static_synapse", "weight": 0.1, "receptor_type": 1})
        )
