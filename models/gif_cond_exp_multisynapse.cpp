...
namespace nest
{
  ...
  nest::gif_cond_exp_multisynapse_dynamics( double, const double* y, double* f, void* pnode )
  {
    ...
    for ( size_t i = 0 ; i < node.P_.num_of_receptors_ ; i++ )
    {
      f[ S::G_EXC + (S::STATE_VEC_SIZE - 1)*i ] = - y[ S::G_EXC + (S::STATE_VEC_SIZE - 1)*i ] / node.P_.tau_syn_[i];
      f[ S::G_INH + (S::STATE_VEC_SIZE - 1)*i ] = - y[ S::G_INH + (S::STATE_VEC_SIZE - 1)*i ] / node.P_.tau_syn_[i];
    }
...
void nest::gif_cond_exp_multisynapse::Parameters_::get(DictionaryDatum &d) const
{
...
void nest::gif_cond_exp_multisynapse::Parameters_::set(const DictionaryDatum& d)
{
  ...
  if (tau_sfa_.size() != q_sfa_.size())
    throw BadProperty(String::compose("'tau_sfa' and 'q_sfa' need to have the same dimensions.\nSize of tau_sfa: %1\nSize of q_sfa: %2", tau_sfa_.size(), q_sfa_.size()));

  if (tau_stc_.size() != q_stc_.size())
    throw BadProperty(String::compose("'tau_stc' and 'q_stc' need to have the same dimensions.\nSize of tau_stc: %1\nSize of q_stc: %2", tau_stc_.size(), q_stc_.size()));
...
void nest::gif_cond_exp_multisynapse::State_::get(DictionaryDatum &d, const Parameters_& p) const
{
  def< double >( d, names::V_m, y_[ V_M ] ); // Membrane potential
  def<double>(d, names::E_sfa, q_); // Adaptive threshold potential
}

void nest::gif_cond_exp_multisynapse::State_::set(const DictionaryDatum& d, const Parameters_& p)
{
...
void nest::gif_cond_exp_multisynapse::init_state_(const Node& proto)
{
  const gif_cond_exp_multisynapse& pr = downcast<gif_cond_exp_multisynapse>(proto);
  S_ = pr.S_;
}

void nest::gif_cond_exp_multisynapse::init_buffers_()
{
...
void nest::gif_cond_exp_multisynapse::calibrate()
{
...
void nest::gif_cond_exp_multisynapse::update(Time const & origin, const long_t from, const long_t to)
{
  ...
  for ( long_t lag = from ; lag < to ; ++lag )
  {
    ...
    for ( size_t i = 0 ; i < P_.num_of_receptors_ ; i++ )
    {
      S_.y_[ State_::G_EXC + (State_::STATE_VEC_SIZE - 1)*i ] +=  B_.spike_exc_[i].get_value( lag );
      S_.y_[ State_::G_INH + (State_::STATE_VEC_SIZE - 1)*i ] +=  B_.spike_inh_[i].get_value( lag );
    }
...
void nest::gif_cond_exp_multisynapse::handle( SpikeEvent& e )
{
...
void nest::gif_cond_exp_multisynapse::handle(CurrentEvent& e)
{
  assert(e.get_delay() > 0);
...
void nest::gif_cond_exp_multisynapse::handle(DataLoggingRequest &e)
{
  B_.logger_.handle(e);
}
