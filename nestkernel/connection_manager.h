...
private:
  ...
  std::vector < std::vector<ConnectorModel*> > prototypes_;          //!< The list of available synapse prototypes: first dimenasion one entry per thread, second dimantion for each synapse type
...
inline
  const ConnectorModel& ConnectionManager::get_synapse_prototype(synindex syn_id, thread t) const
{
...
inline
  void ConnectionManager::assert_valid_syn_id(synindex syn_id, thread t) const
{
