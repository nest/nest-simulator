...
When comparing the efficiency of two models, runtime is often used as a benchmark. Since runtime depends on the number of spikes produced by the underlying dynamics, it can be useful to enforce the same number of spikes in both trials. The ignore-and-spike mechanism supports this by ignoring spikes from the internal dynamics and generating spikes at defined intervals. The input integration and internal dynamics therefore remain intact but have no effect on the output spikes. The mechanism is enabled with ``ignore_and_spike=True`` and configured with ``ignore_and_spike_offset`` and ``ignore_and_spike_interval``. Both values must be larger than zero.
