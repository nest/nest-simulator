...
double
nest::iaf_psc_alpha_presc::Parameters_::set( const DictionaryDatum& d, Node* node )
{
...
void
nest::iaf_psc_alpha_presc::State_::set( const DictionaryDatum& d, const Parameters_& p, double delta_EL, Node* node )
{
...
void
nest::iaf_psc_alpha_presc::update( Time const& origin, const long from, const long to )
{
  assert( to >= 0 );
  ...
  */
  if ( S_.y3_ >= P_.U_th_ )
  {
    ...
    S_.last_spike_offset_ = V_.h_ms_ * ( 1 - std::numeric_limits< double >::epsilon() );
...
nest::iaf_psc_alpha_presc::handle( SpikeEvent& e )
{
  ...
  const long Tdeliver = e.get_rel_delivery_steps( nest::kernel().simulation_manager.get_slice_origin() );

  const double spike_weight = V_.PSCInitialValue_ * e.get_weight() * e.get_multiplicity();
...
nest::iaf_psc_alpha_presc::update_y3_delta_() const
{
  ...
  */
  ...
  const double ps_y2 =
    t_th * ps_e_TauSyn * V_.y1_before_ + ps_e_TauSyn * V_.y2_before_ + t_th * V_.y1_before_ + V_.y2_before_;
...
nest::iaf_psc_alpha_presc::thresh_find_( double const dt ) const
{
  ...
  default:
    LOG( M_ERROR, "iaf_psc_alpha_presc::thresh_find_()", "Invalid interpolation---Internal model error." );
    throw BadProperty();
  }
  return 0;
}
...
nest::iaf_psc_alpha_presc::thresh_find3_( double const dt ) const
{
  ...
  const double w2_ =
    -( 3 * V_.y3_before_ / h_sq ) + ( 3 * S_.y3_ / h_sq ) - ( 2 * deriv_t1 / h_ms_ ) - ( deriv_t2 / h_ms_ );
