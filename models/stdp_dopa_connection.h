...
/* BeginDocumentation
   ...
   from the pool of
                              dopamine releasing neurons and transmitting the
   spikes
                              to the synapse. A value of -1 indicates that no
   volume
...
template < typename targetidentifierT >
inline void
STDPDopaConnection< targetidentifierT >::update_weight_( double_t c0,
  double_t n0,
  double_t minus_dt,
  const STDPDopaCommonProperties& cp )
{
...
template < typename targetidentifierT >
inline void
STDPDopaConnection< targetidentifierT >::facilitate_( double_t kplus,
  const STDPDopaCommonProperties& cp )
{
  c_ += cp.A_plus_ * kplus;
}
...
template < typename targetidentifierT >
inline void
STDPDopaConnection< targetidentifierT >::depress_( double_t kminus,
  const STDPDopaCommonProperties& cp )
{
  ...
  const STDPDopaCommonProperties& cp )
{
  ...
  // trace K_plus to time
  // t_trig
  // but do not increment/decrement as there are no spikes to be handled at
  // t_trig
