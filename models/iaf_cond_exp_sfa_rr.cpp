...
extern "C"
inline int nest::iaf_cond_exp_sfa_rr_dynamics(double, const double y[], double f[], void* pnode)
{ 
  // a shorthand
...
void nest::iaf_cond_exp_sfa_rr::init_state_(const Node& proto)
{
  const iaf_cond_exp_sfa_rr& pr = downcast<iaf_cond_exp_sfa_rr>(proto);
  S_ = pr.S_;
}

void nest::iaf_cond_exp_sfa_rr::init_buffers_()
{
...
void nest::iaf_cond_exp_sfa_rr::calibrate()
{
...
void nest::iaf_cond_exp_sfa_rr::handle(SpikeEvent & e)
{
...
void nest::iaf_cond_exp_sfa_rr::handle(CurrentEvent& e)
{
  assert(e.get_delay() > 0);
...
void nest::iaf_cond_exp_sfa_rr::handle(DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}
