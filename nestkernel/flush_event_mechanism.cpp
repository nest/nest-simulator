...
void
FlushEventMechanism::get_status( DictionaryDatum& d ) const
{
  def< double >( d, names::flush_event_send_interval, flush_event_send_interval_ );
}

void
FlushEventMechanism::set_status( const DictionaryDatum& d )
{
  updateValue< double >( d, names::flush_event_send_interval, flush_event_send_interval_ );
