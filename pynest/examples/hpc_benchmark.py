...
def build_network(logger):
    ...
    nest.Connect(
        nest.FixedIndegree(
            I_neurons, E_neurons, indegree=CI, allow_autapses=False, allow_multapses=True, syn_spec=syn_in
        )
    )
    ...
    nest.Connect(
        nest.FixedIndegree(
            E_neurons, I_neurons, indegree=CE, allow_autapses=False, allow_multapses=True, syn_spec=syn_ex
        )
    )
    ...
    nest.Connect(
        nest.FixedIndegree(
            I_neurons, I_neurons, indegree=CI, allow_autapses=False, allow_multapses=True, syn_spec=syn_in
        )
    )
    ...
    if params["record_spikes"]:
        ...
        nest.Connect(
            nest.AllToAll(local_neurons[: brunel_params["Nrec"]], E_recorder, syn_spec=nest.synapsemodels.static_hpc())
        )
