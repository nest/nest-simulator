...
typedef std::vector< DelayChecker >
  tVDelayChecker; // each thread checks delays themselve

typedef std::vector< size_t > tVCounter; // each synapse type has a counter
typedef std::vector< tVCounter >
  tVVCounter; // and each threads counts for all its synapses
