...
/* BeginDocumentation
  ...
  Description:
   ...
   scheme [1]. When a presynaptic spike occurs, it is taken into account in
   the depression part of the STDP weight change rule with the nearest
   preceding postsynaptic one, and when a postsynaptic spike occurs, it is
   accounted in the facilitation rule with the nearest preceding presynaptic
   one (instead of pairing with all spikes, like in stdp_synapse). For a clear
   illustration of this scheme see fig. 7A in [2].
...
template < typename targetidentifierT >
inline void
STDPNNSymmConnection< targetidentifierT >::send( Event& e,
  thread t,
  double t_lastspike,
  const CommonSynapseProperties& )
{
  ...
  // history(t_lastspike - dendritic_delay, ..., T_spike-dendritic_delay]
...
template < typename targetidentifierT >
STDPNNSymmConnection< targetidentifierT >::STDPNNSymmConnection(
  const STDPNNSymmConnection< targetidentifierT >& rhs )
  : ConnectionBase( rhs )
  , weight_( rhs.weight_ )
...
template < typename targetidentifierT >
void
STDPNNSymmConnection< targetidentifierT >::get_status(
...
template < typename targetidentifierT >
void
STDPNNSymmConnection< targetidentifierT >::set_status( const DictionaryDatum& d,
  ConnectorModel& cm )
{
