...
/**
 * @brief Contact point element for dynamic synapse creation and deletion.
 *
 ...
 *
 * @param z Current number of synaptic elements. Stored as a double variable but
 *           the actual usable number of synaptic elements is an integer truncated
 *           from this double value. A standard value for the growth of a
 *           synaptic element is around 0.0001 elements/ms.
 * @param continuous Defines if the number of synaptic elements should be treated
 *                   as a continuous double number or as an integer value.
 *                   Default is false.
 * @param growth_rate The maximum amount by which the synaptic elements will change
 *                    between time steps. In elements/ms.
 * @param tau_vacant Rate at which vacant synaptic elements will decay. Typical
 *                    is 0.1 which represents a loss of 10% of the vacant synaptic
 *                    elements each time the structural_plasticity_update_interval
 *                    is reached by the simulation time.
 * @param growth_curve Rule which defines the dynamics of this synaptic element.
 *
 * @note Parameters are documented in the class implementation.
 *
 * @see GrowthCurve, SPManager, SPBuilder, Node, ArchivingNode
 *
 * @author Mikael Naveau, Sandra Diaz (July 2013)
 *
