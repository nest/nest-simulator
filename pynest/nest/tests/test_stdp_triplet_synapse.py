...
class STDPTripletConnectionTestCase(unittest.TestCase):
    ...
    def depress(self, w, Kminus, Kplus_triplet):
        """Depress weight."""
        return abs(w - Kminus * (self.syn_spec["Aminus"] + self.syn_spec["Aminus_triplet"] * Kplus_triplet))
    ...
    def test_weightChangeWhenPrePostSpikes(self):
        ...
        weight = np.sign(Wmax) * self.depress(abs(weight), Kminus, Kplus_triplet)
        Kplus += 1.0
        Kplus_triplet += 1.0

        (Kplus, Kplus_triplet, Kminus, Kminus_triplet) = self.decay(2.0 + self.dendritic_delay, Kplus, Kplus_triplet,
                                                                    Kminus, Kminus_triplet)
        weight = np.sign(Wmax) * self.facilitate(abs(weight), Kplus, Kminus_triplet)
        Kminus += 1.0
        Kminus_triplet += 1.0
