...
namespace nest{
  ...
  /* BeginDocumentation
    Name: gif_cond_exp - Conductance based generalized integrate-and-fire neuron model according to Mensi et al. (2012) 
    ...
    C*dV(t)/dt = -g_L*(V(t)-E_L) - etta_1(t) - etta_2(t) - ... - eta_n(t) + I(t)

    where each eta_i is a spike triggered current (stc), and the neuron model can have arbitrary number of them.
  ...
  */
  ...
  inline
port gif_cond_exp::send_test_event(Node& target, rport receptor_type, synindex, bool)
{
  SpikeEvent e;
  e.set_sender(*this);
...
inline
port gif_cond_exp::handles_test_event(SpikeEvent&, rport receptor_type)
{
  if (receptor_type != 0)
...
inline
port gif_cond_exp::handles_test_event(CurrentEvent&, rport receptor_type)
{
  if (receptor_type != 0)
    throw UnknownReceptorType(receptor_type, get_name());
  return 0;
...
inline
port gif_cond_exp::handles_test_event(DataLoggingRequest &dlr, 
				    rport receptor_type)
{
  if (receptor_type != 0)
    throw UnknownReceptorType(receptor_type, get_name());
...
inline
void gif_cond_exp::get_status(DictionaryDatum &d) const
{
  P_.get(d);
  S_.get(d, P_);
...
inline
void gif_cond_exp::set_status(const DictionaryDatum &d)
{
