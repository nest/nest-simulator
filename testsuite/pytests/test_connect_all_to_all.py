...
class TestAllToAll(connect_test_base.ConnectTestBase):
    ...
    def testInputArray(self):
        for label in ["weight", "delay"]:
            ...
            if label == "weight":
                ...
                self.param_array = (
                    np.arange(1, self.N1_array * self.N2_array + 1).reshape(
            ...
            nest.ResetKernel()
            self.setUpNetwork(
                self.conn_dict, syn_params, N1=self.N1_array, N2=self.N2_array
            )
    ...
    def testInputArrayWithoutAutapses(self):
        ...
        for label in ["weight", "delay"]:
            syn_params = {}
            if label == "weight":
                self.param_array = np.arange(self.N1 * self.N1, dtype=float).reshape(
                    self.N1, self.N1
                )
            elif label == "delay":
                self.param_array = (
                    np.arange(1, self.N1 * self.N1 + 1).reshape(self.N1, self.N1) * 0.1
                )
            syn_params[label] = self.param_array
    ...
    def testInputArrayRPort(self):
        ...
        self.param_array = np.transpose(
            np.asarray([np.arange(1, self.N2 + 1) for i in range(self.N1)])
        )
