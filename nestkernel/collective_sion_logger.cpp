...
#include "collective_sion_logger.h"

void
nest::CollectiveSIONLogger::enroll( RecordingDevice& device )
{
  std::vector< Name > value_names;
  nest::CollectiveSIONLogger::enroll( device, value_names );
}
...
void
nest::CollectiveSIONLogger::initialize()
{
  MPI_Comm local_comm;
  int rank;
...
void
nest::CollectiveSIONLogger::finalize()
{
#pragma omp parallel
  {
    Network& network = *( Node::network() );
...
void
nest::CollectiveSIONLogger::synchronize()
{
  Network& network = *( Node::network() );
  thread t = network.get_thread_id();
  int task = network.thread_to_vp( t );
...
void
nest::CollectiveSIONLogger::write( const RecordingDevice& device, const Event& event )
{
...
void
nest::CollectiveSIONLogger::write( const RecordingDevice& device,
  const Event& event,
...
const std::string
nest::CollectiveSIONLogger::build_filename_() const
{
...
nest::CollectiveSIONLogger::SIONBuffer::SIONBuffer()
  : buffer( NULL )
  , ptr( 0 )
  , max_size( 0 )
{
...
nest::CollectiveSIONLogger::SIONBuffer::SIONBuffer( int size )
  : buffer( NULL )
  , ptr( 0 )
{
  if ( size > 0 )
...
void
nest::CollectiveSIONLogger::SIONBuffer::reserve( int size )
{
  if ( size <= max_size )
    return;
...
void
nest::CollectiveSIONLogger::SIONBuffer::ensure_space( int size )
{
  if ( get_free() < size )
  {
...
void
nest::CollectiveSIONLogger::SIONBuffer::write( const char* v, long unsigned int n )
{
  if ( ptr + n <= max_size )
  {
...
int
nest::CollectiveSIONLogger::SIONBuffer::get_size()
{
  return ptr;
}

int
nest::CollectiveSIONLogger::SIONBuffer::get_capacity()
{
  return max_size;
}

int
nest::CollectiveSIONLogger::SIONBuffer::get_free()
{
  return ( max_size - ptr );
}

void
nest::CollectiveSIONLogger::SIONBuffer::clear()
{
  ptr = 0;
}

char*
nest::CollectiveSIONLogger::SIONBuffer::read()
{
  return buffer;
}

template < typename T >
nest::CollectiveSIONLogger::SIONBuffer&
nest::CollectiveSIONLogger::SIONBuffer::operator<<( const T data )
{
...
void
nest::CollectiveSIONLogger::Parameters_::get( const CollectiveSIONLogger& al,
...
void
nest::CollectiveSIONLogger::Parameters_::set( const CollectiveSIONLogger& al,
...
void
nest::CollectiveSIONLogger::set_status( const DictionaryDatum& d )
{
