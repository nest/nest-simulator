...
public:
  NodeInterface();
  NodeInterface( NodeInterface const& );
   ...
   */
  virtual bool supports_urbanczik_archiving() const = 0;

  /**
   * Returns true if the node only receives events from nodes/devices
   * on the same thread.
   */
  virtual bool local_receiver() const = 0;
   ...
   * new nodes to the network.
   */
  virtual bool is_off_grid() const = 0;
   ...
   */
  virtual index get_node_id() const = 0;

  /**
   * Return lockpointer to the NodeCollection that created this node.
   */
  virtual NodeCollectionPTR get_nc() const = 0;
   ...
   */
  virtual void calibrate_time( const TimeConverter& ) = 0;
   ...
   */
  virtual void post_run_cleanup() = 0;
...
public:
   ...
   */
  virtual port
  send_test_event( NodeInterface& receiving_node, rport receptor_type, synindex syn_id, bool dummy_target );
   ...
   */
  virtual double get_Ca_minus() const = 0;
   ...
   * @ingroup SP_functions
   */
  virtual double get_synaptic_elements( Name ) const = 0;
   ...
   * @ingroup SP_functions
   */
  virtual int get_synaptic_elements_vacant( Name ) const = 0;
   ...
   * @ingroup SP_functions
   */
  virtual int get_synaptic_elements_connected( Name ) const = 0;
   ...
   * @ingroup SP_functions
   */
  virtual std::map< Name, double > get_synaptic_elements() const = 0;
   ...
   * @ingroup SP_functions
   */
  virtual void update_synaptic_elements( double ) = 0;
   ...
   * @ingroup SP_functions
   */
  virtual void decay_synaptic_elements_vacant() = 0;
   ...
   * @ingroup SP_functions
   */
  virtual void connect_synaptic_element( Name, int ) = 0;
   ...
   */
  virtual void set_thread( thread ) = 0;

  /**
   * Retrieve the number of the thread to which the node is assigned.
   */
  virtual thread get_thread() const = 0;
   ...
   * compatible information
   */
  virtual SignalType receives_signal() const = 0;
   ...
   */
  virtual DictionaryDatum get_status_base() = 0;
   ...
   * @internal
   */
  virtual void set_status_base( const DictionaryDatum& ) = 0;

  /**
   * Returns true if node is model prototype.
   */
  virtual bool is_model_prototype() const = 0;

  /**
   * set thread local index

   */
  virtual void set_thread_lid( const index ) = 0;

  /**
   * get thread local index
   */
  virtual index get_thread_lid() const = 0;
...
protected:
  virtual void set_node_id_( index ) = 0; //!< Set global node id

  /**
   * Set the original NodeCollection of this node.
   */
  virtual void set_nc_( NodeCollectionPTR ) = 0;
  ...
  virtual void set_initialized_() = 0;

  virtual Model& get_model_() const = 0;

  //! Mark node as frozen.
  virtual void set_frozen_( bool frozen ) = 0;
