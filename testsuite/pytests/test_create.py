...
class CreateTestCase(unittest.TestCase):
    ...
    def test_CopyModel(self):
        ...
        new_syn = nest.CopyModel("static_synapse", weight=10.0)
        nest.Connect(nest.AllToAll(n[0], n[1], syn_spec=new_syn))
        nest.BuildNetwork()
        w = nest.GetDefaults(new_syn.synapse_model)["weight"]
        self.assertEqual(w, 10.0)

        self.assertRaisesRegex(
            nest.kernel.NESTError, "NewModelNameExists", nest.CopyModel, "iaf_psc_alpha", "new_neuron"
        )
