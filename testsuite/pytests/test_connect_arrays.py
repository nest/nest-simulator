...
class TestConnectArrays(unittest.TestCase):
    ...
    def test_connect_arrays_unique(self):
        ...
        nest.Connect(
            nest.ArrayConnect(sources, targets, syn_spec=nest.synapsemodels.static(weight=weights, delay=delays))
        )
    ...
    def test_connect_arrays_rtype(self):
        ...
        nest.Connect(
            nest.ArrayConnect(
                sources,
                targets,
                syn_spec=nest.synapsemodels.static(weight=weights, delay=delays, receptor_type=receptor_type),
            )
        )
    ...
    def test_connect_arrays_additional_synspec_params(self):
        ...
        nest.Connect(
            nest.ArrayConnect(
                sources,
                targets,
                syn_spec=nest.synapsemodels.vogels_sprekeler(
                    weight=weights, delay=delays, receptor_type=receptor_type, alpha=alpha, tau=tau
                ),
            )
        )
        ...
        with self.assertRaises(nest.kernel.NESTErrors.BadParameter):
            nest.Connect(
                nest.ArrayConnect(
                    ...
                    targets,
                    syn_spec=nest.synapsemodels.vogels_sprekeler(
                        weight=weights, delay=delays, receptor_type=receptor_type
                    ),
                )
            )
            nest.BuildNetwork()
    ...
    def test_connect_arrays_receptor_type(self):
        ...
        nest.Connect(
            nest.ArrayConnect(sources, targets, syn_spec=nest.synapsemodels.static(weight=weights, receptor_type=0))
        )
    ...
    def test_connect_arrays_differnt_alpha(self):
        ...
        nest.Connect(
            nest.ArrayConnect(
                source, target, syn_spec=nest.synapsemodels.stdp(weight=weights, receptor_type=0, alpha=alpha)
            )
        )
