...
def test_stdp_synapse_basic():
    """
    Test STDP synapse by reproducing weight changes from spike trains.
    Uses parrot_neuron with background excitatory and inhibitory inputs.
    """
    nest.ResetKernel()
    ...
    # Note: weight is not set in defaults, it's set in the Connect call
    nest.SetDefaults(
        "stdp_synapse",
        {
            "alpha": alpha,
    ...
    w = w_exc / w_max  # normalized weight
