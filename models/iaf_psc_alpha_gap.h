...
#ifndef IAF_PSC_ALPHA_GAP_H
#define IAF_PSC_ALPHA_GAP_H
...
public:
  iaf_psc_alpha_gap();
  iaf_psc_alpha_gap( const iaf_psc_alpha_gap& );
...
private:
  ...
  friend class RecordablesMap< iaf_psc_alpha_gap >;
  friend class UniversalDataLogger< iaf_psc_alpha_gap >;
  ...
  struct Buffers_
  {
    ...
    Buffers_( iaf_psc_alpha_gap& );
    Buffers_( const Buffers_&, iaf_psc_alpha_gap& );
  ...
  static RecordablesMap< iaf_psc_alpha_gap > recordablesMap_;
};

inline void
iaf_psc_alpha_gap::update( Time const& origin, const long from, const long to )
{
  update_( origin, from, to, false );
}

inline bool
iaf_psc_alpha_gap::wfr_update( Time const& origin,
  const long from,
  const long to )
{
...
inline port
iaf_psc_alpha_gap::send_test_event( Node& target,
  rport receptor_type,
  synindex,
  bool )
{
...
inline port
iaf_psc_alpha_gap::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
iaf_psc_alpha_gap::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline port
iaf_psc_alpha_gap::handles_test_event( DataLoggingRequest& dlr,
  rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
iaf_psc_alpha_gap::handles_test_event( GapJunctionEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
iaf_psc_alpha_gap::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d, P_ );
...
inline void
iaf_psc_alpha_gap::set_status( const DictionaryDatum& d )
{
...
#endif /* #ifndef IAF_PSC_ALPHA_GAP_H */
