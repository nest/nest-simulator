...
class StructralPlasticityExample:
    ...
    def connect_external_input(self):
        ...
        nest.Connect(
            nest.AllToAll(noise, self.nodes_e, syn_spec=nest.synapsemodels.static(weight=self.psc_ext, delay=1.0))
        )
        nest.Connect(
            nest.AllToAll(noise, self.nodes_i, syn_spec=nest.synapsemodels.static(weight=self.psc_ext, delay=1.0))
        )
