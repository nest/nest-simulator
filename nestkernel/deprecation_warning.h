...
DeprecationWarning::set_deprecation_warnings()
{
  ...
  //deprecation_warning_["allow_offgrid_spikes"] = true;
  deprecation_warning_.insert( std::pair<std::string, bool>( "allow_offgrid_spikes", true ) );

  assert( deprecation_warning_.find( "allow_offgrid_spikes" )->second == true );

  //std::cerr << "og nå er den vel ikke tom?? " << deprecation_warning_.empty() << " " << deprecation_warning_.size() << std::endl;
}
...
DeprecationWarning::get_deprecation_warning( const std::string param )
{
  ...
  //std::cerr << "er det når vi skal finne deprecation warningen at vi feiler??\n";

  //std::cerr << "er den tom??\n";
  //std::cerr << "tom? " << deprecation_warning_.empty() << " " << deprecation_warning_.size() << std::endl;
  //std::cerr << "deprecation warningen er (begynnelse): \n";
  //std::cerr << deprecation_warning_.find( "allow_offgrid_spikes" )->second << std::endl;

  it = deprecation_warning_.find(param);

  //std::cerr << "er det find?\n";

  //std::cerr << "deprecation warningen er (begynnelse): " << deprecation_warning_[param] << std::endl;
  ...
  //std::cerr << "deprecation warningen er: " << deprecation_warning_[param] << std::endl;
