...
@pytest.mark.parametrize("syn_spec", [{'synapse_model': 'gap_junction'}, {'synapse_model': 'gap_junction', 'weight': 2.0}])
def test_neuron_gap_connect_one_to_one(syn_spec):
    """
    Test if gap junction neurons can be connected by a gap junction synapse with a symmetric one_to_one connection rule.
    """
...
def test_neuron_gap_connect_all_to_all():
    """
    Test if gap junction neurons can be connected by a gap junction synapse with an all_to_all connection rule.
    """
...
def test_neuron_gap_connect_with_delay_fails(conn_spec):
    """
    Test if setting a delay when connecting gap junction neurons via a gap junction synapse leads to an error.
    """
...
def test_neuron_nogap_gap_connect_fails():
    """
    Test if connecting a no-gap junction neuron to a gap junction neuron via a gap junction synapse leads to an error.
    """
    conn_spec = {'rule': 'one_to_one', 'make_symmetric': True}
    syn_spec = {'synapse_model': 'gap_junction'}
    with pytest.raises(nest.kernel.NESTError, match='The target node or synapse model does not support gap junction input.'):
...
def test_neuron_gap_connect_not_symmetric_fails():
    """
    Test if connecting gap junction neurons via a gap junction synapse via a nonsymmetric connection rule leads to an error.
    """
    conn_spec = {'rule': 'one_to_one', 'make_symmetric': False}
    syn_spec = {'synapse_model': 'gap_junction', 'weight': 2.0}
    with pytest.raises(nest.kernel.NESTError, match='Connections with this synapse model can only be created as one-to-one connections with "make_symmetric" set to true *'):
        nest.Connect(pytest.neuron_gap, pytest.neuron_gap, conn_spec=conn_spec, syn_spec=syn_spec)
