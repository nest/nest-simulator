  ...
  const Layer< D >& source )
{
  ...
  {
    if ( allow_autapses_ and ( iter->second == tgt_ptr->get_node_id() ) )
    {
      continue;
    }
  ...
  if ( mask_.get() )
  {
    ...
    for ( NodeCollection::const_iterator tgt_it = target_begin; tgt_it < target_end; ++tgt_it )
    {
      ...
      if ( kernel_.get() )
      {
        ...
        if ( positions.empty()
          or ( allow_autapses_ and ( positions.size() == 1 ) and ( positions[ 0 ].second == target_id ) )
          or ( allow_multapses_ and ( positions.size() < number_of_connections_ ) ) )
        {
        ...
        for ( int i = 0; i < ( int ) number_of_connections_; ++i )
        {
          ...
          if ( allow_multapses_ and ( is_selected[ random_id ] ) )
          {
            --i;
            continue;
          }
  ...
  NodeCollectionPTR target_nc )
{
  ...
  for ( const auto& source_pos_node_id_pair : source_pos_node_id_pairs )
  {
    ...
    if ( target_pos_node_id_pairs.empty()
      or ( allow_multapses_ and ( target_pos_node_id_pairs.size() < number_of_connections_ ) ) )
    {
