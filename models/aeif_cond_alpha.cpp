...
extern "C" int
nest::aeif_cond_alpha_dynamics( double dt, const double y[], double f[], void* pnode )
{
  // a shorthand
  ...
  // limiting rate of change due to exponential term -- prevent numerical instability in the integrator
  f[ S::V_M ] = std::min( I_soma, node.P_.max_V_m_rate_of_change );
  f[ S::V_M ] = std::max( I_soma, -node.P_.max_V_m_rate_of_change );
...
nest::aeif_cond_alpha::Parameters_::Parameters_()
  ...
  , tau_syn_ex( 0.2 )                                                   // ms
  , tau_syn_in( 2.0 )                                                   // ms
  , max_V_m_rate_of_change( std::numeric_limits< double >::infinity() ) // pA
  , I_e( 0.0 )                                                          // pA
  , gsl_error_tol( 1e-6 )
{
}
...
nest::aeif_cond_alpha::Parameters_::get( DictionaryDatum& d ) const
{
  ...
  def< double >( d, names::max_V_m_rate_of_change, max_V_m_rate_of_change );
...
nest::aeif_cond_alpha::Parameters_::set( const DictionaryDatum& d, Node* node )
{
  ...
  updateValueParam< double >( d, names::max_V_m_rate_of_change, max_V_m_rate_of_change, node );
