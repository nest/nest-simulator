...
/** @BeginDocumentation
 ...
 buffer_shrink_limit    double - Shrink spike buffers if last transmission required less than buffer_shrink_limit *
 current size; default 0: no shrinking buffer_shrink_factor    double - When buffer_shrink_limit indicates shrinking,
 shrink to buffer_shrink_factor * current size buffer_grow_extra     double - If buffers must grow, grow to ( 1+
 buffer_grow_extra ) * required size buffer_shrink_count   integer - Number of shrinks that have been performed (read
 only) buffer_grow_count    integer - Number of grows that have been performed (read only) buffer_shrink_delta   integer
 - Total number of elements removed from buffers over all shrink ops, per rank (read only) buffer_grow_delta   integer -
 Total number of elements added to buffers over all grow ops, per rank (read only) keep_source_table booltype    -
 Whether to keep source table after connection setup is complete, defaults to true. local_spike_counter integertype -
 Number of spikes fired by neurons on a given MPI rank during the most recent call to Simulate(). Only spikes from
 “normal” neurons are counted, not spikes generated by devices such as poisson_generator (read only). max_num_syn_models
 integertype - Maximal number of synapse models supported (read only). network_size                          integertype
 - The number of nodes in the network (read only). node_models                           arraytype   - The list of
 available node models (neurons and devices; read only). num_connections                       integertype - The number
 of connections in the network (read only; local only). stimulation_backends                  arraytype   - List of
 available backends for stimulation devices (read-only). structural_plasticity_synapses        dicttype    - Defines all
 synapses which are plastic for the structural plasticity algorithm. Each entry in the dictionary is composed of a
