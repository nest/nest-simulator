...
void RecordablesMap<iaf_chxk_2008>::create() {
	...
	insert_(names::g_ex, &iaf_chxk_2008::get_y_elem_<
			iaf_chxk_2008::State_::G_EXC>);
	insert_(names::g_in, &iaf_chxk_2008::get_y_elem_<
			iaf_chxk_2008::State_::G_INH>);
	insert_(names::g_ahp, &iaf_chxk_2008::get_y_elem_<
			iaf_chxk_2008::State_::G_AHP>);
...
extern "C" inline int nest::iaf_chxk_2008_dynamics(double, const double y[],
		double f[], void* pnode) {
	// a shorthand
...
void nest::iaf_chxk_2008::init_buffers_() {
...
void nest::iaf_chxk_2008::calibrate() {
...
void nest::iaf_chxk_2008::update(Time const & origin, const long_t from,
		const long_t to) {
	...
	for (long_t lag = from; lag < to; ++lag) {
		...
		if ( vm_prev < P_.V_th && S_.y[State_::V_M] >= P_.V_th) {
			...
			double_t sigma = (S_.y[State_::V_M] - P_.V_th) * B_.step_ / (S_.y[State_::V_M] - vm_prev);
...
void nest::iaf_chxk_2008::handle(SpikeEvent & e) {
	assert(e.get_delay() > 0);

	if (e.get_weight() > 0.0)
		B_.spike_exc_.add_value(e.get_rel_delivery_steps(
				network()->get_slice_origin()), e.get_weight()
				* e.get_multiplicity());
	else
...
void nest::iaf_chxk_2008::handle(CurrentEvent& e) {
	assert(e.get_delay() > 0);

	// add weighted current; HEP 2002-10-04
	B_.currents_.add_value(e.get_rel_delivery_steps(
			network()->get_slice_origin()), e.get_weight() * e.get_current());
}

void nest::iaf_chxk_2008::handle(DataLoggingRequest& e) {
	B_.logger_.handle(e);
}
#endif //HAVE_GSL
