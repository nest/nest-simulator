...
template <> void RecordablesMap<aeif_cond_beta_multisynapse>::create() {
  // use standard names wherever you can for consistency!
  insert_(names::V_m, &aeif_cond_beta_multisynapse::get_y_elem_<
                          aeif_cond_beta_multisynapse::State_::V_M>);

  insert_(names::w, &aeif_cond_beta_multisynapse::get_y_elem_<
...
aeif_cond_beta_multisynapse::Parameters_::Parameters_()
    : V_peak_(0.0) // mV, should not be larger that V_th+10
      ...
      V_th(-50.4) // mV
      ,
      num_of_receptors(1), I_e(0.0) // pA
      ,
      gsl_error_tol(1e-6), has_connections_(false) {
...
aeif_cond_beta_multisynapse::State_::State_(const Parameters_ &p)
    : y_(STATE_VECTOR_MIN_SIZE, 0.0), r_(0) {
  y_[0] = p.E_L;
}

aeif_cond_beta_multisynapse::State_::State_(const State_ &s) : r_(s.r_) {
  y_ = s.y_;
}
...
void aeif_cond_beta_multisynapse::Parameters_::get(DictionaryDatum &d) const {
...
void aeif_cond_beta_multisynapse::Parameters_::set(const DictionaryDatum &d) {
  ...
  if (Delta_T < 0.) {
    ...
    if ((V_peak_ - V_th) / Delta_T >= max_exp_arg) {
      throw BadProperty("The current combination of V_peak, V_th and Delta_T"
...
void aeif_cond_beta_multisynapse::State_::get(DictionaryDatum &d) const {
...
void aeif_cond_beta_multisynapse::State_::set(const DictionaryDatum &d) {
...
void aeif_cond_beta_multisynapse::init_state_(const Node &proto) {
  const aeif_cond_beta_multisynapse &pr =
      downcast<aeif_cond_beta_multisynapse>(proto);
  S_ = pr.S_;
}

void aeif_cond_beta_multisynapse::init_buffers_() {
...
void aeif_cond_beta_multisynapse::calibrate() {
  ...
  for (size_t i = 0; i < P_.num_of_receptors; ++i) {
    ...
    if (denom1 != 0) {
      ...
      denom2 =
          std::exp(-t_p / P_.taus_decay[i]) - std::exp(-t_p / P_.taus_rise[i]);
    }
  ...
  S_.y_.resize(
      ...
      (State_::NUMBER_OF_STATES_ELEMENTS_PER_RECEPTOR * P_.num_of_receptors));
 ...
 * ---------------------------------------------------------------- */
void aeif_cond_beta_multisynapse::update(Time const &origin, const long from,
                                         const long to) {
...
port aeif_cond_beta_multisynapse::handles_test_event(SpikeEvent &,
                                                     rport receptor_type) {
  if (receptor_type <= 0 ||
...
void aeif_cond_beta_multisynapse::handle(SpikeEvent &e) {
  if (e.get_weight() < 0) {
    throw BadProperty("Synaptic weights for conductance based models "
                      "must be positive.");
  }
  assert(e.get_delay() > 0);
  assert((e.get_rport() > 0) && ((size_t)e.get_rport() <= P_.num_of_receptors));
...
void aeif_cond_beta_multisynapse::handle(CurrentEvent &e) {
  assert(e.get_delay() > 0);
...
void aeif_cond_beta_multisynapse::handle(DataLoggingRequest &e) {
  B_.logger_.handle(e);
}

extern "C" int aeif_cond_beta_multisynapse_dynamics(double, const double y[],
                                                    double f[], void *pnode) {
  // a shorthand
  ...
  if (node.P_.Delta_T > 0.) {
    I_spike = node.P_.Delta_T * std::exp((V - node.P_.V_th) / node.P_.Delta_T);
  } else {
    I_spike = 0;
  }
