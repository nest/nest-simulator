...
class StdpSpikeMultiplicity(unittest.TestCase):
    ...
    such that K delta < h, i.e., we get K spikes in each time step 
    (h is resolution). If we transmit the same spike trains via plain parrot 
    ...
    precise parrots shall converge to the weight changes observed between 
    the plain parrots.
    ...
    def run_protocol(self, pre_post_shift):
        ...
        for delta in deltas:
            ...
            nest.Connect(pre_parrot + post_parrot 
                         + pre_parrot_ps + post_parrot_ps, spikes)
            ...
            nest.Connect(pre_parrot_ps, post_parrot_ps, 
                         syn_spec={'model': 'stdp_synapse', 'receptor_type': 1})
    ...
    def test_ParrotNeuronSTDPProtocolPotentiation(self):
        ...
        dwrel = dw[1:] / dw[:-1]
        assert all(np.round(dwrel, decimals=3) == 0.5), 'Precise weights do not converge.'
