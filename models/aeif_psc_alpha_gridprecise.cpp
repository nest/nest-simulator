/*
 *  aeif_psc_alpha_gridprecise.cpp
 *
...
nest::RecordablesMap< nest::aeif_psc_alpha_gridprecise > nest::aeif_psc_alpha_gridprecise::recordablesMap_;
...
template <>
void
RecordablesMap< aeif_psc_alpha_gridprecise >::create()
{
  // use standard names whereever you can for consistency!
  insert_( names::V_m, &aeif_psc_alpha_gridprecise::get_y_elem_< aeif_psc_alpha_gridprecise::State_::V_M > );
  insert_( names::I_ex, &aeif_psc_alpha_gridprecise::get_y_elem_< aeif_psc_alpha_gridprecise::State_::I_EXC > );
  insert_( names::I_in, &aeif_psc_alpha_gridprecise::get_y_elem_< aeif_psc_alpha_gridprecise::State_::I_INH > );
  insert_( names::dI_ex, &aeif_psc_alpha_gridprecise::get_y_elem_< aeif_psc_alpha_gridprecise::State_::DI_EXC > );
  insert_( names::dI_in, &aeif_psc_alpha_gridprecise::get_y_elem_< aeif_psc_alpha_gridprecise::State_::DI_INH > );
  insert_( names::w, &aeif_psc_alpha_gridprecise::get_y_elem_< aeif_psc_alpha_gridprecise::State_::W > );
}
}

extern "C" int
nest::aeif_psc_alpha_gridprecise_dynamics( double, const double y[], double f[], void* pnode )
{
  // a shorthand
  typedef nest::aeif_psc_alpha_gridprecise::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const nest::aeif_psc_alpha_gridprecise& node = *( reinterpret_cast< nest::aeif_psc_alpha_gridprecise* >( pnode ) );
  ...
  // dv/dt
  f[ S::V_M ] = ( -node.P_.g_L * ( ( V - node.P_.E_L ) - I_spike ) + I_exc - I_in - w
            + node.P_.I_e + node.B_.I_stim_ ) / node.P_.C_m;

  f[ S::DI_EXC ] = -dI_exc / node.P_.tau_syn_ex;
  f[ S::I_EXC ] = dI_exc - I_exc / node.P_.tau_syn_ex; // Synaptic Conductance (nS)
...
nest::aeif_psc_alpha_gridprecise::Parameters_::Parameters_()
...
nest::aeif_psc_alpha_gridprecise::State_::State_( const Parameters_& p )
  : r_( 0 )
  , r_offset_( 0. )
{
  y_[ 0 ] = p.E_L;
...
nest::aeif_psc_alpha_gridprecise::State_::State_( const State_& s )
  : r_( s.r_ )
...
nest::aeif_psc_alpha_gridprecise::State_& nest::aeif_psc_alpha_gridprecise::State_::operator=( const State_& s )
{
...
void
nest::aeif_psc_alpha_gridprecise::Parameters_::get( DictionaryDatum& d ) const
{
...
void
nest::aeif_psc_alpha_gridprecise::Parameters_::set( const DictionaryDatum& d )
{
...
void
nest::aeif_psc_alpha_gridprecise::State_::get( DictionaryDatum& d ) const
{
...
void
nest::aeif_psc_alpha_gridprecise::State_::set( const DictionaryDatum& d, const Parameters_& )
{
...
nest::aeif_psc_alpha_gridprecise::Buffers_::Buffers_( aeif_psc_alpha_gridprecise& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::aeif_psc_alpha_gridprecise::Buffers_::Buffers_( const Buffers_&, aeif_psc_alpha_gridprecise& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::aeif_psc_alpha_gridprecise::aeif_psc_alpha_gridprecise()
  : Archiving_Node()
  , P_()
  , S_( P_ )
  , B_( *this )
...
nest::aeif_psc_alpha_gridprecise::aeif_psc_alpha_gridprecise( const aeif_psc_alpha_gridprecise& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::aeif_psc_alpha_gridprecise::init_state_( const Node& proto )
{
  const aeif_psc_alpha_gridprecise& pr = downcast< aeif_psc_alpha_gridprecise >( proto );
  S_ = pr.S_;
}

void
nest::aeif_psc_alpha_gridprecise::init_buffers_()
{
  ...
  B_.sys_.function = aeif_psc_alpha_gridprecise_dynamics;
...
void
nest::aeif_psc_alpha_gridprecise::calibrate()
{
...
void
nest::aeif_psc_alpha_gridprecise::interpolate_( double& t, double t_old )
{
...
void
nest::aeif_psc_alpha_gridprecise::spiking_( const long_t lag, const double t )
{
  // spike event
  SpikeEvent se;
...
void
nest::aeif_psc_alpha_gridprecise::update( const Time& origin, const nest::long_t from, const nest::long_t to )
{
...
void
nest::aeif_psc_alpha_gridprecise::handle( SpikeEvent& e )
{
...
void
nest::aeif_psc_alpha_gridprecise::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void
nest::aeif_psc_alpha_gridprecise::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
