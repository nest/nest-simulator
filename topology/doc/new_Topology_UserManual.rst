...
The Topology Module provides the NEST simulator [1]_ [2] with a
convenient interface for creating layers of neurons placed in space and
connecting neurons in such layers with probabilities and properties
...
Python interface to NEST [1]. NEST users using the SLI interface should
be able to map instructions to corresponding SLI code. This manual is
not meant as a comprehensive reference manual. Please consult the online
documentation in PyNEST for details; where appropriate, that
documentation also points to relevant SLI documentation.
...
.. figure:: user_manual_figures/layer1.png
   :alt: Figure 1: Simple grid-based layer centered about the origin.
   Blue circles mark layer elements, the thin square the extent of the
   layer. Row and column indices are shown in the right and top margins,
   respectively.
   :name: fig:layer1
...
The layer is shown in Fig. \ `1 <#fig:layer1>`__. Note the following
properties:
...
-  The *grid spacing* of the layer is
   ...
   \ In the layer shown, we have :math:`dx=dy=0.2`, but the grid spacing
   may differ in x- and y-direction.
...
-  Each element of a grid-based layer has a *row- and column-index* in
   ...
   top and right margin of Fig. \ `1 <#fig:layer1>`__. Note that
   row-indices follow matrix convention, i.e., run from top to bottom.
...
.. figure:: user_manual_figures/layer2.png
   :alt: Figure 2: Same layer as in Fig. `1 <#fig:layer1>`__, but with
   different extent.
   :name: fig:layer2

   Figure 2: Same layer as in Fig. `1 <#fig:layer1>`__, but with
   different extent.

The resulting layer is shown in Fig. `2 <#fig:layer2>`__. The extent is
always a two-element tuple of floats. In this example, we have grid
spacings :math:`dx=0.4` and :math:`dy=0.1`. Changing the extent does not
affect grid indices.
...
.. figure:: user_manual_figures/layer3.png
   :alt: Figure 3: Three layers centered, respectively, about
   :math:`(0,0)` (blue), :math:`(-1,-1)` (green), and :math:`(1.5,0.5)`
   (red).
   :name: fig:layer3

   Figure 3: Three layers centered, respectively, about :math:`(0,0)`
...
Fig. \ `3 <#fig:layer3>`__, grid indices run from 0 to 4 through columns
and rows, respectively, even though elements in these three layers have
...
From Eq. \ `1 <#eq:dx_dy_extent>`__, we see that the extent of the layer
must be :math:`(n_c d, n_r d)`. We now need to find the coordinates
...
and shown in Fig. `4 <#fig:layer3a>`__:
...
.. figure:: user_manual_figures/layer3a.png
   :alt: Figure 4: Layer with :math:`n_c=5` rows and :math:`n_r=3`
   columns, spacing :math:`d=0.1` and the left edge of the extent at
   :math:`x=0`, centered about the :math:`y`-axis. The cross marks the
   point on the extent placed at the origin :math:`(0,0)`, the circle
   the center of the layer.
   :name: fig:layer3a

   Figure 4: Layer with :math:`n_c=5` rows and :math:`n_r=3` columns,
...
.. figure:: user_manual_figures/layer4.png
   :alt: Figure 5: A free layer with 50 elements uniformly distributed
   in an extent of size :math:`1\times 1`.
   :name: fig:layer4
...
.. figure:: user_manual_figures/layer4_3d.png
   :alt: Figure 6: A free 3D layer with 200 elements uniformly
   distributed in an extent of size :math:`1\times 1\times 1`.
   :name: fig:layer4_3d

   Figure 6: A free 3D layer with 200 elements uniformly distributed in
   an extent of size :math:`1\times 1\times 1`.
...
network. In the :math:`5\times 5`-layer in Fig. `1 <#fig:layer1>`__,
...
surface of a torus. Fig. \ `7 <#fig:player>`__ illustrates this for a
...
.. figure:: user_manual_figures/player.png
   :alt: Figure 7: Top left: Layer with single row and five columns
   without periodic boundary conditions. Numbers above elements show
   element coordinates. Colors shifting from blue to magenta mark
   increasing distance from the element at :math:`(-2,0)`. Bottom left:
   Same layer, but with periodic boundary conditions. Note that the
   element at :math:`(2,0)` now is a nearest neighbor to the element at
...
Here is a simple example, cf. \ `8 <#fig:conn1>`__
...
.. figure:: user_manual_figures/conn1.png
   :alt: Figure 8: Left: Minimal connection example from a layer onto
   itself using a rectangular mask shown as red line for the node at
   :math:`(0,0)` (marked light red). The targets of this node are marked
   with red dots. The targets for the node at :math:`(4,5)` are marked
   with yellow dots. This node has fewer targets since it is at the
   corner and many potential targets are beyond the layer. Right: The
   effect of periodic boundary conditions is seen here. Source and
   target layer and connection dictionary were identical, except that
   periodic boundary conditions were used. The node at :math:`(4,5)` now
   has 15 targets, too, but they are spread across the corners of the
   layer. If we wrapped the layer to a torus, they would form a
   :math:`5\times 3` rectangle centered on the node at :math:`(4,5)`.
   :name: fig:conn1

   Figure 8: Left: Minimal connection example from a layer onto itself
   using a rectangular mask shown as red line for the node at
   :math:`(0,0)` (marked light red). The targets of this node are marked
   with red dots. The targets for the node at :math:`(4,5)` are marked
   with yellow dots. This node has fewer targets since it is at the
   corner and many potential targets are beyond the layer. Right: The
...
Fig. \ `9 <#fig:conn2_a>`__. The masks are
...
.. figure:: user_manual_figures/conn2_a.png
   :alt: Figure 9: Masks for 2D layers. For all mask types, the driver
   node is marked by a wide light-red circle, the selected pool nodes by
   red dots and the masks by red lines. From left to right, top to
   bottom: rectangular, circular, doughnut and elliptical masks centered
...
examples (cf. Fig. `10 <#fig:conn2_b>`__).
...
.. figure:: user_manual_figures/conn2_b.png
   :alt: Figure 10: The same masks as in Fig. `9 <#fig:conn2_a>`__, but
   centered about :math:`(-1.5,-1.5)`, :math:`(-2,0)`, :math:`(1.5,1.5)`
   and :math:`(2, -1)`, respectively, using the ``'anchor'`` parameter.
   :name: fig:conn2_b

   Figure 10: The same masks as in Fig. `9 <#fig:conn2_a>`__, but
   centered about :math:`(-1.5,-1.5)`, :math:`(-2,0)`, :math:`(1.5,1.5)`
   and :math:`(2, -1)`, respectively, using the ``'anchor'`` parameter.

and :math:`\textbf{elliptical}` masks, see Fig \ `10 <#fig:conn2_b>`__.
To do so, add an ``'azimuth_angle'`` entry in the specific mask
dictionary. The ``azimuth_angle`` is measured in degrees and is the
rotational angle from the x-axis to the y-axis.
...
.. figure:: user_manual_figures/conn2_c.png
   :alt: Figure 11: Rotated rectangle and elliptical mask from
   Fig. \ `9 <#fig:conn2_a>`__ and Fig. \ `10 <#fig:conn2_b>`__, where
   the rectangle mask is rotated :math:`120^\circ` and the elliptical
   mask is rotated :math:`45^\circ`.
   :name: fig:conn2_c
...
.. figure:: user_manual_figures/conn_3d.png
   :alt: Figure 12: Masks for 3D layers. For all mask types, the driver
   node is marked by a wide light-red circle, the selected pool nodes by
   red dots and the masks by red lines. From left to right: box and
   spherical masks centered about the driver node.
   :name: fig:conn3d

   Figure 12: Masks for 3D layers. For all mask types, the driver node
   is marked by a wide light-red circle, the selected pool nodes by red
...
The resulting connections are shown in Fig. \ `13 <#fig:conn3>`__. By
default the top-left corner of a grid mask, i.e., the grid mask element
with grid index :math:`[0,0]`\  [4]_, is aligned with the driver node.
...
.. figure:: user_manual_figures/conn3.png
   :alt: Figure 13: Grid masks for connections between grid-based
   layers. Left: :math:`5\times 3` mask with default alignment at upper
   left corner. Center: Same mask, but anchored to center node at grid
   index :math:`[1,2]`. Right: Same mask, but anchor to the upper left
   of the mask at grid index :math:`[-1,2]`.
   :name: fig:conn3

   Figure 13: Grid masks for connections between grid-based layers.
   Left: :math:`5\times 3` mask with default alignment at upper left
   corner. Center: Same mask, but anchored to center node at grid index
...
Available kernel functions are shown in Table \ `1 <#tbl:kernels>`__.
More kernel functions may be created in a NEST extension module. This is
covered in Chapter \ `5 <#ch:extending>`__.

.. table:: Table 1: Functions currently available in the Topology
module. :math:`d` is the distance and :math:`(d_x,d_y)` the
displacement. All functions can be used to specify weights and delays,
but only the constant and the distance-dependent functions, i.e., all
functions above the double line, can be used as kernels.
...
.. figure:: user_manual_figures/conn4.png
   :alt: Figure 14: Illustration of various kernel functions. Top left:
   constant kernel, :math:`p=0.5`. Top center: Gaussian kernel, green
   dashed lines show :math:`\sigma`, :math:`2\sigma`, :math:`3\sigma`.
   Top right: Same Gaussian kernel anchored at :math:`(1.5,1.5)`. Bottom
...
The functions presented in Table \ `1 <#tbl:kernels>`__ can also be used
to specify distance-dependent or randomized weights and delays for the
connections created by ``ConnectLayers``.

Figure \ `15 <#fig:conn5>`__ illustrates weights and delays generated
using these functions with the following code examples. All examples use
a “layer” of 51 nodes placed on a line; the line is centered about
:math:`(25,0)`, so that the leftmost node has coordinates :math:`(0,0)`.
The distance between neighboring elements is 1. The mask is rectangular,
...
Results are shown in the top panel of Fig. \ `15 <#fig:conn5>`__.
Connection weights and delays are shown for the leftmost neuron as
driver. Weights drop linearly from :math:`1`. From the node at
:math:`(20,0)` on, the cutoff sets weights to 0. There are no
connections to nodes beyond :math:`(25,0)`, since the mask extends only
25 units to the right of the driver. Delays increase in a stepwise
linear fashion, as NEST requires delays to be multiples of the
...
Results are shown in the middle panel of Fig. \ `15 <#fig:conn5>`__.
...
about the node at :math:`(0,0)` wraps back to the right half of the
...
Results are shown in the bottom panel of Fig. `15 <#fig:conn5>`__. It
shows linear, exponential and Gaussian weight functions for the node at
:math:`(25,0)`.
...
circles in the bottom panel of Fig. `15 <#fig:conn5>`__. Weights and
delays can currently only be randomized with uniform distribution.

.. figure:: user_manual_figures/conn5.png
   :alt: Figure 15: Distance-dependent and randomized weights and
   delays. See text for details.
   :name: fig:conn5
...
The resulting distribution of distances between connected nodes is shown
in Fig. \ `16 <#fig:conn6>`__.
...
.. figure:: user_manual_figures/conn6.png
   :alt: Figure 16: Distribution of distances between source and target
   for a network of 1000 randomly placed nodes, a fixed fan out of 50
...
.. figure:: user_manual_figures/vislayer.png
   :alt: Figure 17: :math:`21\times 21` grid with divergent Gaussian
   projections onto itself. Blue circles mark layer elements, red
   circles connection targets of the center neuron (marked by large
   light-red circle). The large red circle is the mask, the dashed green
   lines mark :math:`\sigma`, :math:`2\sigma` and :math:`3\sigma` of the
   Gaussian kernel.
   :name: fig:vislayer

   Figure 17: :math:`21\times 21` grid with divergent Gaussian
   projections onto itself. Blue circles mark layer elements, red
   circles connection targets of the center neuron (marked by large
   light-red circle). The large red circle is the mask, the dashed green
   lines mark :math:`\sigma`, :math:`2\sigma` and :math:`3\sigma` of the
...
The resulting graphics is shown in Fig. `17 <#fig:vislayer>`__. All
elements and the targets of the center neuron are shown, as well as mask
and kernel.
...
``{.c++ language="C++"} #include "topologymodule.h" #include "parameter.h"``

Then, add the class definition, e.g. near the bottom of the file before
the brace closing the namespace ``mynest``:

\``\` {.c++ language=“C++”} class Affine2DParameter: public
nest::Parameter { public: Affine2DParameter(const DictionaryDatum& d):
Parameter(d), a_(1.0), b_(1.0), c_(0.0) { updateValue(d, “a”, a_);
updateValue(d, “b”, b_); updateValue(d, “c”, c_); }
...
``{.c++ language="C++"} nest::TopologyModule::register_parameter<Affine2DParameter>("affine2d");``
...
\``\` {.c++ language=“C++”} class EllipticMask : public nest::Mask<2> {
public: EllipticMask(const DictionaryDatum& d): rx_(1.0), ry_(1.0) {
updateValue(d, “r_x”, rx_); updateValue(d, “r_y”, ry_); }
...
``{.c++ language="C++"}     nest::TopologyModule::register_mask<EllipticMask>("elliptic");``
...
.. container:: references
   ...
   .. container::
      ...
      1. Eppler JM, Helias M, Muller E et al. (2008). PyNEST: A
      convenient interface to the NEST simulator. Front Neuroinformatics
      2 12. doi:
      `10.3389/neuro.11.012.2008 <https://doi.org/10.3389/neuro.11.012.2008>`__.
   ...
   .. container::
      ...
      3. Nordlie E, Gewaltig M-O, Plesser HE (2009). Towards
      reproducible descriptions of neuronal network models. PLoS Comput
      Biol 5 (8): e1000456. doi:
      `10.1371/journal.pcbi.1000456 <https://doi.org/10.1371/journal.pcbi.1000456>`__.
...
.. [2]
   See [3](456) for suggestions on how to describe network models.
