...
void
nest::step_current_generator::Parameters_::set( const DictionaryDatum& d, Buffers_& b )
{
  ...
  if ( !amp_times_.empty() )
  {
    ...
    for ( std::vector< double_t >::const_iterator next = prev + 1; next != amp_times_.end();
          ++next, ++prev )
      if ( *prev >= *next )
...
void
nest::step_current_generator::update( Time const& origin, const long_t from, const long_t to )
{
  ...
  for ( long_t offs = from; offs < to; ++offs )
  {
    ...
    if ( B_.idx_ < P_.amp_times_.size()
      && curr_time + 1 == Time( Time::ms( P_.amp_times_[ B_.idx_ ] ) ).get_steps() )
    {
