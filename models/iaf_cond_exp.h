...
namespace nest
{
  ...
  inline
port nest::iaf_cond_exp::send_test_event(Node& target, rport receptor_type, synindex, bool)
{
  SpikeEvent e;
  e.set_sender(*this);
...
inline
port iaf_cond_exp::handles_test_event(SpikeEvent&, rport receptor_type)
{
  if (receptor_type != 0)
...
inline
port iaf_cond_exp::handles_test_event(CurrentEvent&, rport receptor_type)
{
  if (receptor_type != 0)
    throw UnknownReceptorType(receptor_type, get_name());
  return 0;
...
inline
port iaf_cond_exp::handles_test_event(DataLoggingRequest& dlr, 
				       rport receptor_type)
{
  if (receptor_type != 0)
    throw UnknownReceptorType(receptor_type, get_name());
  ...
  inline
  void iaf_cond_exp::get_status(DictionaryDatum &d) const
  {
    P_.get(d);
    S_.get(d);
  ...
  inline
  void iaf_cond_exp::set_status(const DictionaryDatum &d)
  {
