...
nest::volume_transmitter::calibrate()
{
  ...
  B_.spikecounter_.reserve( kernel().connection_builder_manager.get_min_delay() * P_.deliver_interval_ + 1 );
}
...
nest::volume_transmitter::update( const Time&, const long_t from, const long_t to )
{
    ...
    == 0 )
  {
    ...
    if ( !B_.spikecounter_.empty() )
      kernel().connection_builder_manager.trigger_update_weight( get_gid(), B_.spikecounter_, t_trig );
