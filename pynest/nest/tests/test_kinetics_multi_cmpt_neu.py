...
class mc_neuron_ode():
        ...
        def fun_u(self, t, y):
                f = [1. / self.C_m[0, 0] * (-self.G_l[0, 0]\
                     * (y[0] - self.E_l[0, 0]) - y[12]\
                     * (y[0] - self.E_ex[0, 0])\
                     - y[14] * (y[0] - self.E_in[0, 0]) - self.G_conn[0]\
                     * ((y[0] - self.E_l[0, 0]) - (y[1] - self.E_l[1, 0]))\
                     + self.I_curr[0, 0]),\
                     1. / self.C_m[1, 0] * (-self.G_l[1, 0]\
                     * (y[1] - self.E_l[1, 0])\
                     - y[8] * (y[1] - self.E_ex[1, 0]) - y[10]\
                     * (y[1] - self.E_in[1, 0])\
                     - self.G_conn[0] * ((y[1] - self.E_l[1, 0])\
                     - (y[0] - self.E_l[0, 0])) - self.G_conn[1]\
                     * ((y[1] - self.E_l[1, 0]) - (y[2] - self.E_l[2, 0]))\
                     + self.I_curr[1, 0]),\
                     1. / self.C_m[2, 0] * (-self.G_l[2, 0]\
                     * (y[2] - self.E_l[2, 0])\
                     - y[4] * (y[2] - self.E_ex[2, 0]) - y[6]\
                     * (y[2] - self.E_in[2, 0])\
                     - self.G_conn[1] * ((y[2] - self.E_l[2, 0])\
                     - (y[1] - self.E_l[1, 0]))\
                     + self.I_curr[2, 0] + y[15] * y[16]\
                     * self.gca * (self.eca - y[2])),\
                     ...
                     (1.0 / (1.0 + np.exp((y[2] - self.halfm)\
                     * -1.0 * self.slopem)) - y[15]) / self.taum,\
                     (1.0 / (1.0 + np.exp((y[2] - self.halfh)\
                     * -1.0 * self.slopeh)) - y[16]) / self.tauh]
        ...
        def __init__(self, active, resolution):
                ...
                self.Conn_mat = np.array([[self.G_conn[0],\
                -self.G_conn[0], 0.], [-self.G_conn[0], self.G_conn[0]\
                + self.G_conn[1] + self.E_l[1], -self.G_conn[1]],\
                [0., -self.G_conn[1], self.G_conn[1] + self.E_l[2]]])
                ...
                self.u = np.array([-70.0, -65.0, -60.0, 0., 0.,\
                ...
                self.y_exc = np.array([[[0.], [0.]], [[0.],\
                [0.]], [[0.], [0.]]])
                self.y_inh = np.array([[[0.], [0.]], [[0.],\
                [0.]], [[0.], [0.]]])
                self.y_ca = np.array([[[0.], [0.]], [[0.],\
                [0.]], [[0.], [0.]]])
                ...
                self.slopeh = -0.5
                self.u[15] = 1.0\
                / (1.0 + np.exp((self.u[2] - self.halfm) * -1.0 * self.slopem))
                self.u[16] = 1.0\
                / (1.0 + np.exp((self.u[2] - self.halfh) * -1.0 * self.slopeh))
                self.r = ode(self.fun_u).set_integrator('dop853',\
                rtol=1e-3, nsteps=500)
...
class KineticsMultiCmptNeuTestCase(unittest.TestCase):
    ...
    def test_KineticsMultiCmptNeu(self):
        ...
        for t in range(0, l):
                tmp_exc = amp_exc * exc_wt * np.reshape(exc_input_arr[:, t],\
                (tot_exc, 1))
                tmp_inh = amp_inh * inh_wt * np.reshape(inh_input_arr[:, t],\
                (tot_inh, 1))
                ...
                ca_arr[t] = neur.u[15] * neur.u[16]\
                * neur.gca * (neur.eca - neur.u[2])
        ...
        nest.Connect([sgs[0]], n, syn_spec={'receptor_type':\
        syns['distal_exc'], "weight": amp_exc, "delay": 1.0})
