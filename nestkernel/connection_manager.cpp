...
void ConnectionManager::delete_connections_()
{
  for (tVSConnector::iterator it = connections_.begin(); it != connections_.end(); ++it)
    for (tSConnector::nonempty_iterator iit = it->nonempty_begin(); iit != it->nonempty_end(); ++iit)
#ifdef USE_PMA
      (*iit)->~ConnectorBase();
#else
      delete (*iit);
...
{
  for (std::vector< std::vector< ConnectorModel* > >::iterator it = prototypes_.begin(); it != prototypes_.end(); ++it)
  {
...
synindex ConnectionManager::register_synapse_prototype(ConnectorModel * cf)
{
...
synindex ConnectionManager::copy_synapse_prototype(synindex old_id, std::string new_name)
{
  ...
  {
    net_.message(SLIInterpreter::M_ERROR, "ConnectionManager::copy_synapse_prototype", "CopyModel cannot generate another synapse. Maximal synapse model count of 255 exceeded.");
...
void ConnectionManager::set_prototype_status(synindex syn_id, const DictionaryDatum& d)
{
  ...
  for (thread t=0; t<net_.get_num_threads(); ++t)
  {
    ...
    {
      throw BadProperty(String::compose( "Setting status of prototype '%1': %2", prototypes_[t][syn_id]->get_name(), e.message()));
    }
  }
}

DictionaryDatum ConnectionManager::get_prototype_status(synindex syn_id) const
{
  assert_valid_syn_id(syn_id);
...
DictionaryDatum ConnectionManager::get_synapse_status(index gid, synindex syn_id, port p, thread tid)
{
  assert_valid_syn_id(syn_id);
...
void ConnectionManager::set_synapse_status(index gid, synindex syn_id, port p, thread tid, const DictionaryDatum& dict)
{
  assert_valid_syn_id(syn_id);
  try
  {
    connections_[tid].get(gid)->set_synapse_status(syn_id, *(prototypes_[tid][syn_id]), dict, p);
  }
  catch (BadProperty& e) 
  {
    throw BadProperty(String::compose( "Setting status of '%1' connecting from GID %2 to port %3: %4", prototypes_[tid][syn_id]->get_name(), gid, p, e.message()));
  }
}
...
#pragma omp parallel
    ...
    for (thread t = 0; t < net_.get_num_threads(); ++t)
    {
      ...
      for(tSConnector::const_nonempty_iterator it=connections_[t].nonempty_begin(); it != connections_[t].nonempty_end(); ++it)
      {
...
#pragma omp critical
      ...
      for (index source_id=1; source_id< connections_[t].size(); ++source_id)
      {
        if(connections_[t].get(source_id) != 0)
          connections_[t].get(source_id)->get_connections(source_id, t, syn_id,conns_in_thread);
      }
      ...
      {  
		if (connections_[t].get(source_id) != 0)
        {
          for (index t_id=0; t_id< target->size(); ++t_id)
          {
            ...
            connections_[t].get(source_id)->get_connections(source_id, target_id, t, syn_id, conns_in_thread);
          }
        }
      }
      if (conns_in_thread.size()>0)
...
ConnectorBase* ConnectionManager::validate_source_entry(thread tid, index s_gid, synindex syn_id)
{
  assert_valid_syn_id(syn_id);
  ...
  if (connections_[tid].test(s_gid))
    return connections_[tid].get(s_gid);   // returns non-const reference to stored type, here ConnectorBase*
  else
...
void ConnectionManager::connect(Node& s, Node& r, index s_gid, thread tid, index syn, double_t d, double_t w)
{
...
void ConnectionManager::connect(Node& s, Node& r, index s_gid, thread tid, index syn, DictionaryDatum& p, double_t d, double_t w)
{
  // see comment above for explanation
  ConnectorBase* conn = validate_source_entry(tid, s_gid, syn);
...
void ConnectionManager::trigger_update_weight(const long_t vt_id, const vector<spikecounter>& dopa_spikes, const double_t t_trig)
{
  for (thread t = 0; t < net_.get_num_threads(); ++t)
    for(tSConnector::const_nonempty_iterator it=connections_[t].nonempty_begin(); it != connections_[t].nonempty_end(); ++it)
...
size_t ConnectionManager::get_num_connections() const
{
  num_connections_ = 0;
  for (thread t=0; t<net_.get_num_threads(); ++t)
    for (std::vector<ConnectorModel*>::const_iterator i = prototypes_[t].begin(); i != prototypes_[t].end(); ++i) 
