...
void nest::gif_cond_exp::Parameters_::get(DictionaryDatum &d) const
{
...
void nest::gif_cond_exp::Parameters_::set(const DictionaryDatum& d)
{
...
void nest::gif_cond_exp::State_::get(DictionaryDatum &d, const Parameters_& p) const
{
  def< double >( d, names::V_m, y_[ V_M ] ); // Membrane potential
  def<double>(d, names::E_sfa, q_); // Adaptive threshold potential
}

void nest::gif_cond_exp::State_::set(const DictionaryDatum& d, const Parameters_& p)
{
...
void nest::gif_cond_exp::init_state_(const Node& proto)
{
  const gif_cond_exp& pr = downcast<gif_cond_exp>(proto);
  S_ = pr.S_;
}

void nest::gif_cond_exp::init_buffers_()
{
...
void nest::gif_cond_exp::calibrate()
{
...
void nest::gif_cond_exp::update(Time const & origin, const long_t from, const long_t to)
{
...
void nest::gif_cond_exp::handle(SpikeEvent & e)
{
...
void nest::gif_cond_exp::handle(CurrentEvent& e)
{
  assert(e.get_delay() > 0);
...
void nest::gif_cond_exp::handle(DataLoggingRequest &e)
{
  B_.logger_.handle(e);
}
