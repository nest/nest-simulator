...
# using the GLIF parameters of cell 490626718 in Allen Cell Type Database( ACTD ).
# The parameters is available at
...
def create_lif_psc(mechanisms, syn_tau):
    ...
    return nest.Create("glif_psc", 
                       params={"C_m": 58.72, 
                               ...
                               "t_ref": 3.75, 
                               "tau_syn": syn_tau,
                               "spike_dependent_threshold": mechanisms[0], # False
                               "after_spike_currents": mechanisms[1], # False
                               "adapting_threshold": mechanisms[2]}) # False

def create_lif_r_psc(mechanisms, syn_tau):
    ...
    return nest.Create("glif_psc",
                       params={"C_m": 58.72, 
                               ...
                               "spike_dependent_threshold": mechanisms[0], # True
                               "after_spike_currents": mechanisms[1], # False
                               "adapting_threshold": mechanisms[2]}) # False

def create_lif_asc_psc(mechanisms, syn_tau):
    ...
    return nest.Create("glif_psc",
                       params={"C_m": 58.72, 
                               ...
                               "tau_syn": syn_tau,
                               "spike_dependent_threshold": mechanisms[0], # False
                               "after_spike_currents": mechanisms[1], # True
                               "adapting_threshold": mechanisms[2]}) # False

def create_lif_r_asc_psc(mechanisms, syn_tau):
...
def create_lif_r_asc_a_psc(mechanisms, syn_tau):
    ...
    return nest.Create("glif_psc",
                       params={"C_m": 58.72, 
                               ...
                               "spike_dependent_threshold": mechanisms[0], # True
                               "after_spike_currents": mechanisms[1], # True
                               "adapting_threshold": mechanisms[2]}) # True
...
# for the picked model.We store the returned handle in variable for later reference. 
...
mechanisms = model_mech_dict[glif_model]
    
if glif_model == "lif":
    neuron = create_lif_psc(mechanisms, syn_tau)
elif glif_model == "lif_r":
    neuron = create_lif_r_psc(mechanisms, syn_tau)
elif glif_model == "lif_asc":
    neuron = create_lif_asc_psc(mechanisms, syn_tau)
elif glif_model == "lif_r_asc":
    neuron = create_lif_r_asc_psc(mechanisms, syn_tau)
elif glif_model == "lif_r_asc_a":
    neuron = create_lif_r_asc_a_psc(mechanisms, syn_tau)
else:
    neuron = create_lif_psc(mechanisms, syn_tau)
...
# three spikes; we also create one step current generator and a poisson generator,
# a parrot neuron( to be paired with the poisson generator ).
...
# definition of the start and stop times and the amplitude of the injected current.
# Configuration of the poisson generator includes the definition of the start
# and stop times and the rate of the injected spike train.

espikes = nest.Create("spike_generator", params={"spike_times": [
                      10., 100., 150.], "spike_weights": [20.]*3})
ispikes = nest.Create("spike_generator", params={"spike_times": [
                      15., 99., 150.], "spike_weights": [-20.]*3})
cg = nest.Create("step_current_generator", params = {"amplitude_values":[400., ],
                                                     "amplitude_times":[200., ], 
                                                     "start":200., "stop":500.})
pg = nest.Create("poisson_generator", params = {"rate":150000., "start":600., "stop":900.})
...
nest.Connect(espikes, neuron, syn_spec = {"delay":resolution, "receptor_type":1})
nest.Connect(ispikes, neuron, syn_spec = {"delay":resolution, "receptor_type":1})    
...
mm = nest.Create("multimeter", params={"interval":resolution, "record_from": 
                                       ["V_m", "I", "I_syn", "threshold",
                                        "threshold_spike", 
...
# the sum of after spike currents( in cyan ), and the synaptic currents( in magenta )
# in responding to the spike inputs to the neuron in the third panel.
