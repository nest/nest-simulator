...
def brunel_tsodyks_network(
    ...
    nodes_exc = nest.Create('iaf_psc_exp',
                            model_params['NE'],
                            params=neuron_params
                            )

    # Create inhibitory nodes
    nodes_inh = nest.Create('iaf_psc_exp',
                            model_params['NI'],
                            params=neuron_params
                            )
    ...
    random_vm = nest.random.normal(model_params['mean_potential'],
                                   model_params['sigma_potential'])
    ...
    # Create Tsodyk synapses
    nest.SetDefaults('tsodyks_synapse', {'delay': model_params['D'],
    ...
    nest.Connect(pre=nodes_exc,
                 post=espikes,
                 conn_spec='all_to_all',
                 syn_spec='syn_exc'
                 )

    nest.Connect(pre=nodes_inh,
                 post=ispikes,
                 conn_spec='all_to_all',
                 syn_spec='syn_exc'
                 )
...
if __name__ == "__main__":
    ...
    sim_params = {
        "T_sim": 500.,      # total simulation time [ms]
        "T_presim": 0.,    # equilibrating simulation time [ms]
    ...
    sts_exc, sts_inh = brunel_tsodyks_network(n_procs=n_procs,
                                              n_threads=n_threads,
