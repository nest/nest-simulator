...
This script simulates a finite network of generalized integrate-and-fire (GIF)
neurons directly on the mesoscopic population level using the effective
...
Note that the population model represents the mesoscopic level description of 
the corresponding microscopic network based on the NEST model gif_psc_exp.
'''
...
J_syn = np.array([[ J, -g * J], [J, -g * J]]) * C0 / C #final synaptic weights
                                                       #scaling as 1/C

taus1_ = [3., 6.]  #time constants of exc. and inh. post-synaptic currents 
                   #(PSC's)
...
To directly simulate the mesoscopic population activities (i.e. generating the
activity of a finite-size population without simulating single neurons), we can
build the populations using the NEST model gif_pop_psc_exp:
'''
...
'''
To record the instantaneous population rate Abar(t) we use a multimeter, and to
get the population activity A_N(t) we use spike detector: 
'''
...
for i, nest_i in enumerate( nest_pops ):
    ...
    data_sd = nest.GetStatus(nest_sd[i], \
        keys=['events'])[0][0]['times'] * dt - t0
...
As mentioned above, the population model gif_pop_psc_exp directly simulates the
mesoscopic population activities, i.e. without the need to simulate single
neurons. On the other hand, if we want to know single neuron activities, we must
simulate on the microscopic level. This is possible by building a corresponding
network of gif_psc_exp neuron models:
'''
...
mesoscopic population activities A_N(t) from the microscopic simulation. We
also record the membrane potentials of five example neurons: 
'''
...
for i, nest_i in enumerate( nest_pops ):
    ...
    nest.SetStatus(nest_mm_Vm[i], {'record_from':['V_m'], \
                                   ...
                                   'interval': dt_rec})
    nest.Connect(nest_mm_Vm[i], list( np.array(nest_pops[i])[:Nrecord[i]]), \
        'all_to_all')


'''
As before, all neurons in a given population will be stimulated with a step
input current. The following code block is identical to the one for the
mesoscopic simulation above:
'''
...
'''
This should look similar to the population activity obtained from the mesoscopic
simulation based on the NEST model gif_pop_psc_exp (cf. figure 1). Now we
retrieve the data of the multimeter, which allows us to look at the membrane
potentials of single neurons. Here we plot the voltage traces (in mV) of five
example neurons:
'''
...
for i in range(M):
    if Nrecord[i]>0:
        ...
        voltage.append( np.array([v[np.where(senders==j)] \
            for j in set(senders)]) )
    else:
