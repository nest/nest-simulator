/*
 *  iaf_cond_alpha_multisynapse.cpp
 *
...
nest::RecordablesMap< nest::iaf_cond_alpha_multisynapse >
  nest::iaf_cond_alpha_multisynapse::recordablesMap_;
...
template <>
void
RecordablesMap< iaf_cond_alpha_multisynapse >::create()
{
  // use standard names wherever you can for consistency!
  insert_( names::V_m,
    &iaf_cond_alpha_multisynapse::
      get_y_elem_< iaf_cond_alpha_multisynapse::State_::V_M > );
}
...
extern "C" int
iaf_cond_alpha_multisynapse_dynamics( double,
  const double y[],
  double f[],
  void* pnode )
{
  ...
  typedef nest::iaf_cond_alpha_multisynapse::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const nest::iaf_cond_alpha_multisynapse& node =
    *( reinterpret_cast< nest::iaf_cond_alpha_multisynapse* >( pnode ) );
...
iaf_cond_alpha_multisynapse::State_::State_( const Parameters_& p )
...
iaf_cond_alpha_multisynapse::State_::State_( const State_& s )
  : r_( s.r_ )
{
  y_ = s.y_;
}

iaf_cond_alpha_multisynapse::State_& iaf_cond_alpha_multisynapse::State_::
...
void
iaf_cond_alpha_multisynapse::Parameters_::get( DictionaryDatum& d ) const
{
...
void
iaf_cond_alpha_multisynapse::Parameters_::set( const DictionaryDatum& d )
{
  ...
  if ( V_reset_ >= V_th )
  {
    throw BadProperty( "Ensure that: V_reset < V_th ." );
  }
...
void
iaf_cond_alpha_multisynapse::State_::get( DictionaryDatum& d ) const
{
...
void
iaf_cond_alpha_multisynapse::State_::set( const DictionaryDatum& d )
{
  updateValue< double >( d, names::V_m, y_[ V_M ] );
}

iaf_cond_alpha_multisynapse::Buffers_::Buffers_(
  iaf_cond_alpha_multisynapse& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
iaf_cond_alpha_multisynapse::Buffers_::Buffers_( const Buffers_& b,
  iaf_cond_alpha_multisynapse& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
...
iaf_cond_alpha_multisynapse::iaf_cond_alpha_multisynapse()
  : Archiving_Node()
  , P_()
  , S_( P_ )
  , B_( *this )
...
iaf_cond_alpha_multisynapse::iaf_cond_alpha_multisynapse(
  const iaf_cond_alpha_multisynapse& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
iaf_cond_alpha_multisynapse::init_state_( const Node& proto )
{
  const iaf_cond_alpha_multisynapse& pr =
    downcast< iaf_cond_alpha_multisynapse >( proto );
  S_ = pr.S_;
}

void
iaf_cond_alpha_multisynapse::init_buffers_()
{
  ...
  B_.sys_.function = iaf_cond_alpha_multisynapse_dynamics;
...
void
iaf_cond_alpha_multisynapse::calibrate()
{
 ...
 * ---------------------------------------------------------------- */
void
iaf_cond_alpha_multisynapse::update( Time const& origin,
  const long from,
  const long to )
{
  assert(
...
port
iaf_cond_alpha_multisynapse::handles_test_event( SpikeEvent&,
  rport receptor_type )
{
  if ( receptor_type <= 0
...
void
iaf_cond_alpha_multisynapse::handle( SpikeEvent& e )
{
  if ( e.get_weight() < 0 )
  {
    throw BadProperty(
...
void
iaf_cond_alpha_multisynapse::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void
iaf_cond_alpha_multisynapse::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
