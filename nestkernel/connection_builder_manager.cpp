...
DictionaryDatum
nest::ConnectionBuilderManager::get_synapse_status( index gid,
  synindex syn_id,
  port p,
  thread tid )
{
  ...
  index syn )
{
  ...
  else if ( target_ptr->local_receiver() ) // normal devices
  {
    ...
    if ( ( source_ptr->get_thread() != target_thread )
      && ( source_ptr->has_proxies() ) )
    {
  ...
  {
    if ( !source_ptr->has_proxies() ) // we do not allow to connect a device to
                                      // a global receiver at the moment
      return false;
    const thread n_threads = kernel().vp_manager.get_num_threads();
    for ( thread t = 0; t < n_threads; t++ )
    {
  ...
  index syn )
{
  ...
  // for DoubleVectorDatum copies the data into an array, from which the data
  // must then be copied once more.
...
nest::ConnectorBase*
nest::ConnectionBuilderManager::validate_source_entry_( thread tid,
  index s_gid,
  synindex syn_id )
{
  kernel().model_manager.assert_valid_syn_id( syn_id );
...
void
nest::ConnectionBuilderManager::convergent_connect(
  ...
  index syn )
{
  ...
  for ( index i = 0; i < source_ids.size(); ++i )
  {
    ...
    // right one now. As get_node() is quite expensive, so we only call it if we
    // need to
    // if (source->get_thread() != target_thread)
...
void
nest::ConnectionBuilderManager::random_convergent_connect(
  const TokenArray& source_ids,
  index target_id,
  index n,
  const TokenArray& weights,
