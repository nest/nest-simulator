...
nest::ArchivingNode::set_spiketime( Time const& t_sp, double offset )
{
  ...
  if ( n_incoming_ )
  {
    ...
    while ( history_.size() > 1 )
    {
      ...
      if ( history_.front().access_counter_ >= n_incoming_ and t_sp_ms - next_t_sp > max_delay_ +
            kernel().connection_manager.get_min_delay() + kernel().connection_manager.get_stdp_eps() )
      {
        history_.pop_front();
      }
      else
