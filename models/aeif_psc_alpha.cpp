...
nest::aeif_psc_alpha::State_::get( dictionary& d ) const
{
  ...
  d[ names::dI_syn_ex.toString() ] = y_[ DI_EXC ];
  d[ names::I_syn_in.toString() ] = y_[ I_INH ];
  d[ names::dI_syn_in.toString() ] = y_[ DI_INH ];
...
nest::aeif_psc_alpha::State_::set( const dictionary& d, const Parameters_&, Node* node )
{
  ...
  update_value_param( d, names::dI_syn_ex.toString(), y_[ DI_EXC ], node );
  update_value_param( d, names::I_syn_in.toString(), y_[ I_INH ], node );
  update_value_param( d, names::dI_syn_in.toString(), y_[ DI_INH ], node );
