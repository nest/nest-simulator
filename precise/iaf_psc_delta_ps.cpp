...
void
nest::iaf_psc_delta_ps::State_::get( DictionaryDatum& d,
...
void
nest::iaf_psc_delta_ps::State_::set( const DictionaryDatum& d,
  const Parameters_& p,
  double delta_EL )
{
...
nest::iaf_psc_delta_ps::Buffers_::Buffers_( const Buffers_&,
  iaf_psc_delta_ps& n )
  : logger_( n )
{
}
...
void
iaf_psc_delta_ps::update( Time const& origin,
  const long from,
  const long to )
{
  assert( to >= 0 );
...
void
nest::iaf_psc_delta_ps::emit_spike_( Time const& origin,
  const long lag,
...
void
nest::iaf_psc_delta_ps::emit_instant_spike_( Time const& origin,
  const long lag,
