...
inline void
ConnectionManager::send_to_devices( const thread tid, const size_t source_node_id, Event& e )
{
  target_table_devices_.send_to_device( tid, source_node_id, e, kernel().model_manager.get_connection_models( tid ) );
}

inline void
ConnectionManager::send_to_devices( const thread tid, const size_t source_node_id, SecondaryEvent& e )
{
  target_table_devices_.send_to_device( tid, source_node_id, e, kernel().model_manager.get_connection_models( tid ) );
}

inline void
ConnectionManager::send_from_device( const thread tid, const size_t ldid, Event& e )
{
